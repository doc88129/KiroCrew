...
def test_the_mask_is_materialised_so_it_applies_on_a_fresh_install():
    ...
    # Membership in the tuple IS the property: ``_materialize_maskable_dirs`` walks
    # exactly this list before every namespace spawn, and
    # ``test_sandbox_absent_ceiling_seal.TestMaskableDirsAreMaterializedBeforeTheSpawn``
    # asserts each of its leaves is created owner-only, behind its own POSIX gate.
    assert agent_panel._PANELS_DIRNAME in sandbox._CREW_PRECREATE_HIDDEN_DIR_LEAVES
    # Pin the consumer too, so moving the list to a function that does not
    # materialise it fails here rather than silently dropping the mask.
    assert "_CREW_PRECREATE_HIDDEN_DIR_LEAVES" in inspect.getsource(
