...
class TestPreserveOnUnconsumed:
    ...
    @pytest.mark.asyncio
    async def test_teardown_does_not_preserve_an_advisory_the_user_opted_out_of(self, state):
        """The steer landed, the user set the popover to Disabled before the
        turn consumed it: the teardown must not stage that advice for the
        next (disabled) turn. The steered row is relabelled as dropped."""
        from kiro_crew.advisor.delivery import ADVISOR_STATE_DROPPED_BY_OPT_OUT
        ...
        rows = [m for m in slot.messages if m.get("role") == "advisor"]
        assert [m["meta"]["advisorState"] for m in rows] == [ADVISOR_STATE_DROPPED_BY_OPT_OUT]

    def test_hard_kill_drops_staged_advice_too(self, state):
        """A preserve that landed just before the second Stop press stays
        staged otherwise; the kill means discard everything, advice included."""
        from kiro_crew.advisor.delivery import (
        ...
        slot = _running_slot(state)
        _stage_pending_context(state, slot, "late advice")
        slot.append(
            "advisor",
            "late advice",
            "msg msg-advisor",
            meta={"advisorUpdateId": "u9", "advisorState": ADVISOR_STATE_PRESERVED},
        )
        drop_pending_advisor_context(state, slot, to_state="discarded")
        assert not slot._advisor_pending_context
        assert slot.messages[-1]["meta"]["advisorState"] == "discarded"

    @pytest.mark.asyncio
    async def test_a_steer_landing_after_revocation_is_discarded_not_kept(self, state):
        """The opt-out (or a slot rebind) lands while the steer RPC is in
        flight: the text already reached the live turn and cannot be unsent,
        but nothing of it may stay -- the row flips to discarded, the pending
        registration is retired, nothing is staged, and the caller learns the
        advisory was revoked rather than delivered."""
        from kiro_crew.advisor.delivery import ADVISORY_REVOKED_AFTER_STEER

        slot = _running_slot(state)
        authorized = {"ok": True}

        async def steer(message):
            authorized["ok"] = False  # revoked while the RPC is suspended
            return True
        ...
        client.steer = steer
        slot._acp_client = client
        outcome = await deliver_advisory(
            state, slot, make_note(), make_envelope(), proceed=lambda: authorized["ok"]
        )
        # Distinct from a plain revocation: the text DID reach the turn, so the
        # dispatcher must account for it as an interruption.
        assert outcome == ADVISORY_REVOKED_AFTER_STEER
        # The slot may front another conversation by now: the row is gone from
        # the transcript, and live clients get a discarded, blanked patch.
        ...
        assert patches and patches[-1][1]["meta"]["advisorState"] == "discarded"
        assert patches[-1][1]["meta"]["advisorText"] == ""
        # ...and the flag that tells them to drop the row rather than render a
        # blank card under the stopped-turn label.
        assert patches[-1][1]["meta"]["advisorRemoved"] is True
        assert slot._pending_steers == [] and slot._advisory_envelopes == {}
        assert not slot._advisor_pending_context
...
class TestAdvisorContextDrain:
    ...
    def test_staged_entries_are_redacted_again_at_the_provider_egress(self):
        """Staged text is redacted when it is produced, but it also round-trips
        through persisted session metadata; an entry tampered on disk to carry a
        credential or an exfiltration URL must still leave through the same two
        outbound redactors every advisory surface applies."""
        ...
        slot = SimpleNamespace(
            _advisor_pending_context=[
                "[nit] token AKIAIOSFODNN7EXAMPLE leaked, see "
                "https://evil.example/collect?data=QUtJQUlPU0ZPRE5ON0VYQU1QTEU6c2VjcmV0S2V5MTIzNDU2Nzg5MDEyMzQ1Ng=="
            ]
        )
        out = _peek_advisor_context(state, slot, "user asks something")
        assert "AKIAIOSFODNN7EXAMPLE" not in out
        assert "collect?data=" not in out and "REDACTED: suspicious URL" in out
...
class TestEveryResumePathHydratesTheAdvisorMeta:
    """A session that opted out (``advisor_override = "off"``) or carries staged
    advice must come back that way through EVERY path that publishes a slot
    from disk, not only the history rehydration: a resume that left the default
    (``inherit``) would observe a session its user opted out of."""

    @pytest.mark.asyncio
    async def test_api_chat_slot_resume_restores_override_and_staged_context(
        ...
        monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
        state = _make_state(tmp_path)
        key = "dashboard:optout"
        state.conversation_log.update_metadata(
            key,
            {
                "title": "opted out",
                "advisor_override": "on",
                "advisor_pending_context": ["Blocker: keep the lock"],
            },
        )
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.post("/api/chat/slots/optout/resume", json={"key": key})
        assert resp.status == 200
        slot = state._slots["optout"]
        assert slot.advisor_override == "on"
        assert slot._advisor_pending_context == ["Blocker: keep the lock"]
        assert slot._advisor_pending_context_key == effective_session_key(slot)

    @pytest.mark.asyncio
    async def test_api_chat_slot_resume_collapses_a_malformed_override_to_off(
        self, tmp_path, monkeypatch
    ):
        from aiohttp.test_utils import TestClient, TestServer
        from chat_test_helpers import _make_app, _make_state
        ...
        key = "dashboard:bad"
        state.conversation_log.update_metadata(
            key, {"title": "t", "advisor_override": "yes please"}
        )
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.post("/api/chat/slots/bad/resume", json={"key": key})
        assert resp.status == 200
        assert state._slots["bad"].advisor_override == "off"

    def test_surface_channel_session_restores_the_override(self, tmp_path):
        from chat_test_helpers import _make_state
        ...
        slot = channel_slots.surface_channel_session(
            state,
            {"key": key, "title": "thread"},
            {"advisor_override": "on", "advisor_pending_context": ["saved advice"]},
            [{"role": "user", "content": "hi"}],
            session_key=key,
        )
        assert slot is not None
        assert slot.advisor_override == "on"
        assert slot._advisor_pending_context == ["saved advice"]
        assert slot._advisor_pending_context_key
...
class TestCommitMarksSlotDirty:
    ...
    def test_duplicate_staged_texts_are_all_removed_by_the_commit(self):
        """The peek reads each text once, so two staged copies of one text (two
        notes identical after redaction) inject once; the commit must remove
        every copy or the survivor re-injects the same advice next turn."""
        from kiro_crew.advisor.delivery import (
            commit_peeked_advisor_context,
            peek_pending_advisor_context,
        )
        ...
        slot = MagicMock()
        slot._advisor_pending_context = ["advice A", "advice A", "advice B"]
        slot._advisor_peeked_context = peek_pending_advisor_context(slot)
        assert slot._advisor_peeked_context == ["advice A", "advice B"]
        commit_peeked_advisor_context(slot)
        assert slot._advisor_pending_context == []
