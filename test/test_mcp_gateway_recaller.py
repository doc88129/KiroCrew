...
@pytest.mark.asyncio
async def test_member_proof_uses_kernel_peer_and_is_renewed_per_tool_call(monkeypatch):
    issued = []
    ...
    backend, audit = await _run(
        [register, _CALL, {**_CALL, "method": "resources/list"}, _CALL], monkeypatch
    )
    assert issued == [("dashboard:reviewer", 8001)] * 2
...
async def _run(
    frames: list[dict[str, Any]], monkeypatch: pytest.MonkeyPatch, *, writer: _FakeWriter | None = None
    ...
    await asyncio.wait_for(
        gw._handle_connection(
            _FakeReader(frames), writer or _FakeWriter(), pool=_FakePool(),
...
@pytest.mark.asyncio
@pytest.mark.parametrize("claimed,protected", [
    ("dashboard:global", "dashboard:member-reviewer"),
    ("", "dashboard:member-reviewer"),
    ("dashboard:member-reviewer", ""),
    ("", ""),
])
async def test_protected_peer_cannot_downgrade_to_v1_or_omit_its_caller(monkeypatch, claimed, protected):
    issued = []
    loop_thread = threading.get_ident()
    ...
    writer = _FakeWriter()
    backend, _audit = await _run([_register(claimed), _CALL], monkeypatch, writer=writer)
...
async def test_a_protected_peer_never_continues_without_its_proof(monkeypatch, failure):
    ...
    monkeypatch.setattr(gw, "issue_member_session_proof", issue)
    writer = _FakeWriter()
    backend, _audit = await _run([_register("dashboard:reviewer"), _CALL], monkeypatch, writer=writer)
    assert backend.callers == []
...
async def test_protected_identity_is_rechecked_after_a_runtime_rekey(monkeypatch):
    ...
    writer = _FakeWriter()
    backend, _audit = await _run([_register("dashboard:reviewer"), _CALL, {**_CALL, "id": 2}], monkeypatch, writer=writer)
...
@pytest.mark.asyncio
async def test_an_unowned_legacy_v1_caller_keeps_working_without_a_proof(monkeypatch):
    monkeypatch.setattr(socketsec, "get_peer_pid", lambda _writer: 8001)
    ...
    backend, _audit = await _run([_register("dashboard:global"), _CALL], monkeypatch)
