...
@pytest.mark.parametrize(
    "settings", [{}, {"embedding_threads": 999, "embedding_bulk_threads": 999}]
)
def test_loaded_saved_config_preserves_the_effective_resource_budget(
    ...
    assert (cfg.memory.embedding_threads, cfg.memory.embedding_bulk_threads) == before
    cfg.save()
    assert (emb._embed_threads(), emb.bulk_embed_threads()) == before
    assert before == ((2, 2) if settings else (1, 1))
