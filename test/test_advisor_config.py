...
def _fresh():
    ...
    # The backend confirmation stays closed until configure_from_config sees
...
class TestDisableDetachesInheritedObservers:
    ...
    @pytest.mark.asyncio
    async def test_detached_inherited_session_is_released_through_the_pool(self):
        """The observer goes, and so does its pool session: the release cancels
        a review already transmitted and lets the runtime reap once the
        explicit-on sessions are gone. The explicit-on session is untouched."""
        import asyncio
        ...
        service._pool = _Pool()
        service.attach("dashboard:a", override="inherit")
        service.attach("dashboard:b", override="on")
        configure_from_config(
            SimpleNamespace(
                advisor=SimpleNamespace(enabled=False),
                agent=SimpleNamespace(role_models={"advisor": ""}),
        ...
        await asyncio.sleep(0)  # the release is scheduled fire-and-forget
        assert released == ["dashboard:a"]


class TestSpecIsAlwaysMaterializedFromThePackage:
    """The packaged reviewer spec is the only source and is rewritten unchanged
    on every launch. A stale managed file, hand edit, or planted link cannot
    widen the toolless boundary."""
...
class TestStartupConfigureDiscardsStaleSnapshot:
    ...
    def test_configure_from_config_bumps_epoch(self):
        ...
        service = service_mod._service = AdvisorService(enabled=False, reviewer_available=True)
