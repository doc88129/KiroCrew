...
class TestOrphanedObserversAreReclaimed:
    """Slots leave the registry through more than ``close_slot``: OpenAI-compatible
    ephemeral completions, fork/transfer failure cleanups and history deletion
    pop them directly. An observer keyed by a session none of the live slots
    fronts any more is dead weight holding retained evidence and a pool
    session, so the service reclaims those on every attach, through the same
    terminal boundary a close fires."""

    def test_attach_reclaims_observers_with_no_live_slot(self):
        live = {"dashboard:a", "dashboard:b"}
        service = AdvisorService(enabled=True, reviewer_available=True)
        service.live_session_keys = lambda: set(live)
        service.attach("dashboard:a")
        service.attach("dashboard:ephemeral-1")
        # Creating the next observer reclaims ephemeral-1 (no live slot fronts it).
        service.attach("dashboard:ephemeral-2")
        assert set(service._observers) == {"dashboard:a", "dashboard:ephemeral-2"}
        service.attach("dashboard:b")
        assert set(service._observers) == {"dashboard:a", "dashboard:b"}
        assert service.observer_count() == 2, "every orphan was reclaimed on attach"

    def test_the_session_being_attached_is_never_treated_as_an_orphan(self):
        service = AdvisorService(enabled=True, reviewer_available=True)
        service.live_session_keys = lambda: set()  # the slot is published after attach
        assert service.attach("dashboard:new") is not None
        assert service.observer_count() == 1

    def test_without_a_liveness_source_nothing_is_reclaimed(self):
        service = AdvisorService(enabled=True, reviewer_available=True)
        service.attach("dashboard:a")
        service.attach("dashboard:b")
        assert service.observer_count() == 2

    def test_the_gateway_wires_the_slot_registry_as_the_liveness_source(self):
        """``_kick_advisor_configure`` registers ``live_session_keys`` from the
        dashboard state, so the reclaim is live in production and not only in
        tests that set the hook by hand."""
        import inspect

        from kiro_crew.dashboard import server

        source = inspect.getsource(server._kick_advisor_configure)
        assert "live_session_keys" in source and "effective_session_key" in source
