...
class TestAdvisorOverrideEndpoint:
    ...
    async def test_on_is_refused_while_the_reviewer_is_unavailable(self, state, _patch_sel):
        ...
        service_mod._service = AdvisorService(enabled=False, reviewer_available=True)
        service_mod._service.reviewer_available = False  # a non-kiro backend
        slot = state.get_or_create_slot("test")
        try:
            async with TestClient(TestServer(_make_app_with_override(state))) as client:
                resp = await client.post(
...
class TestOverrideOffDropsStagedAdvice:
    """A user who turns the Advisor off has disagreed with its pending advice.

    Leaving the staged context in place would inject it into their next
    message under a popover that reads "Disabled", and the card would keep
    saying "Saved for next turn". Off drops the staged context of every
    alias slot of the session and flips the preserved cards to a dropped
    state the client can label; the effectively-on values are inert.
    """

    def _stage(self, state, slot, text):
        from kiro_crew.advisor.delivery import ADVISOR_STATE_PRESERVED, _stage_pending_context

        _stage_pending_context(state, slot, text)
        slot.append(
            "advisor",
            text,
            "msg msg-advisor",
            meta={"advisorUpdateId": f"u-{text}", "advisorState": ADVISOR_STATE_PRESERVED},
        )

    @pytest.mark.asyncio
    async def test_off_drops_staged_advice_and_marks_the_cards(self, state, _patch_sel):
        from kiro_crew.advisor.delivery import (
            ADVISOR_STATE_DROPPED_BY_OPT_OUT,
            peek_pending_advisor_context,
        )

        _kiro_service()
        slot = state.get_or_create_slot("test")
        self._stage(state, slot, "pending advice")
        patches: list[dict] = []
        state.broadcast_ws = lambda kind, payload: patches.append({"kind": kind, **payload})
        async with TestClient(TestServer(_make_app_with_override(state))) as client:
            resp = await client.post(
                "/api/chat/slots/test/advisor-override", json={"advisor_override": "off"}
            )
            assert resp.status == 200
        assert peek_pending_advisor_context(slot) == []
        card = [r for r in slot.messages if r.get("role") == "advisor"][-1]
        assert card["meta"]["advisorState"] == ADVISOR_STATE_DROPPED_BY_OPT_OUT
        # Live clients rendered the card as "Saved for next turn": they get the patch.
        assert any(
            p["kind"] == "chat_message_update"
            and p["meta"]["advisorState"] == ADVISOR_STATE_DROPPED_BY_OPT_OUT
            for p in patches
        )

    @pytest.mark.asyncio
    @pytest.mark.parametrize("value", ["on", "inherit"])
    async def test_effectively_on_values_keep_staged_advice(self, state, _patch_sel, value):
        from kiro_crew.advisor.delivery import ADVISOR_STATE_PRESERVED, peek_pending_advisor_context
        ...
        async with TestClient(TestServer(_make_app_with_override(state))) as client:
            resp = await client.post(
                "/api/chat/slots/test/advisor-override", json={"advisor_override": value}
            )
            assert resp.status == 200
        assert peek_pending_advisor_context(slot) == ["pending advice"]
        card = [r for r in slot.messages if r.get("role") == "advisor"][-1]
        assert card["meta"]["advisorState"] == ADVISOR_STATE_PRESERVED

    def test_only_advice_still_staged_is_relabelled(self, state):
        """A preserved card whose advice a later turn already consumed keeps
        its label: the opt-out drops what is pending, not delivered history."""
        from kiro_crew.advisor.delivery import (
            ADVISOR_STATE_DROPPED_BY_OPT_OUT,
            ADVISOR_STATE_PRESERVED,
            commit_peeked_advisor_context,
            drop_pending_advisor_context,
            peek_pending_advisor_context,
        )

        slot = state.get_or_create_slot("test")
        self._stage(state, slot, "consumed advice")
        slot._advisor_peeked_context = peek_pending_advisor_context(slot)
        commit_peeked_advisor_context(slot)  # the next turn took it
        self._stage(state, slot, "still pending")
        # The same text preserved again later: only the newer, still-pending
        # card is relabelled; the delivered one keeps its label.
        self._stage(state, slot, "consumed advice")
        drop_pending_advisor_context(state, slot)
        states = [
            (r["content"], r["meta"]["advisorState"])
            for r in slot.messages
            if r.get("role") == "advisor"
        ]
        assert states == [
            ("consumed advice", ADVISOR_STATE_PRESERVED),
            ("still pending", ADVISOR_STATE_DROPPED_BY_OPT_OUT),
            ("consumed advice", ADVISOR_STATE_DROPPED_BY_OPT_OUT),
        ]

    @pytest.mark.asyncio
    async def test_a_failed_save_keeps_the_staged_advice_and_its_cards(self, state, _patch_sel):
        """The drop is part of the opt-out write: a save that fails answers 500
        and rolls the override back, so the advice and its cards stay too."""
        from kiro_crew.advisor.delivery import ADVISOR_STATE_PRESERVED, peek_pending_advisor_context
        ...
        card = [r for r in slot.messages if r.get("role") == "advisor"][-1]
        assert card["meta"]["advisorState"] == ADVISOR_STATE_PRESERVED
        assert not [p for p in patches if p["kind"] == "chat_message_update"]

    def test_a_restored_opted_out_slot_carries_no_staged_advice(self, state):
        """Crash window: the override landed on disk as off before the flush
        persisted the drop. Restoring the slot applies the same rule."""
        from kiro_crew.dashboard.chat_persistence import hydrate_advisor_meta

        slot = state.get_or_create_slot("restored")
        hydrate_advisor_meta(
            slot, {"advisor_override": "off", "advisor_pending_context": ["stale advice"]}
        )
        assert slot.advisor_override == "off"
        assert not getattr(slot, "_advisor_pending_context", [])
        on = state.get_or_create_slot("kept")
        hydrate_advisor_meta(on, {"advisor_override": "on", "advisor_pending_context": ["advice"]})
        assert on._advisor_pending_context == ["advice"]

    def test_drop_is_session_scoped_and_persists_the_row(self, state):
        """Alias slots of one session share the drop; the mutation is flushed."""
        from kiro_crew.advisor.delivery import (
            ADVISOR_STATE_DROPPED_BY_OPT_OUT,
            drop_pending_advisor_context,
            peek_pending_advisor_context,
        ...
        a = state.get_or_create_slot("a")
        b = state.get_or_create_slot("b")
        a.linked_session_key = b.linked_session_key = "slack:1700000000.000200"
        self._stage(state, a, "advice on a")
        self._stage(state, b, "advice on b")
        a._dirty = b._dirty = False
        drop_pending_advisor_context(state, a)
        assert peek_pending_advisor_context(a) == []
        assert peek_pending_advisor_context(b) == []
        for s in (a, b):
            card = [r for r in s.messages if r.get("role") == "advisor"][-1]
            assert card["meta"]["advisorState"] == ADVISOR_STATE_DROPPED_BY_OPT_OUT
...
class TestOverrideReauthorizedInsideLock:
    ...
    def test_reauthorization_is_inside_the_lock(self):
        ...
        assert inside.find("_app_cancel_denied") < inside.find(
            "set_advisor_override(member, override)"
        )
...
class TestOverrideIsSessionScopedAcrossAliasSlots:
    ...
    @pytest.mark.asyncio
    async def test_a_failed_save_does_not_roll_back_an_alias_that_changed_meanwhile(
        self, state, _patch_sel
    ):
        """Alias B rebinds to another session and commits its own ``off`` while
        A's save is still awaiting; when A's save then fails, the rollback may
        restore only what this request itself still owns -- B's acknowledged
        opt-out on its new session must stand."""
        _kiro_service()
        a = state.get_or_create_slot("alias-a")
        b = state.get_or_create_slot("alias-b")
        a.linked_session_key = b.linked_session_key = "slack:1700000000.000100"
        ...
        async def failing_save(st, s_, *args, **kw):
            # The rebind + B's own commit (its own transaction) land during the await.
            b.linked_session_key = "slack:1700000000.000999"
            b.advisor_override = "off"
            b._advisor_override_txn += 1
            raise RuntimeError("disk full")
        ...
        with patch("kiro_crew.dashboard.chat_handlers.save_slot_off_loop", failing_save):
            async with TestClient(TestServer(_make_app_with_override(state))) as client:
                ...
                assert resp.status == 500
        assert a.advisor_override == "on", "the authorized slot rolls back"
        assert b.advisor_override == "off", "the rebound alias keeps its own acknowledged opt-out"

    @pytest.mark.asyncio
    async def test_a_sibling_that_committed_the_same_value_meanwhile_keeps_it(
        self, state, _patch_sel
    ):
        """ABA: during A's save, alias B rebinds to another session and commits
        the SAME value A wrote (``off``) for that session through its own
        request. Value equality cannot tell A's write from B's, so the
        post-save rebind restore must key on a per-slot mutation token and
        leave B's acknowledged value alone."""
        _kiro_service()
        a = state.get_or_create_slot("alias-a")
        b = state.get_or_create_slot("alias-b")
        a.linked_session_key = b.linked_session_key = "slack:1700000000.000100"
        ...
        async def save_then_sibling_commits(st, s_, *args, **kw):
            b.linked_session_key = "slack:1700000000.000999"
            # B's own endpoint request landed: same value, its own transaction.
            b.advisor_override = "off"
            b._advisor_override_txn += 1
            return True

        with patch(
            "kiro_crew.dashboard.chat_handlers.save_slot_off_loop", save_then_sibling_commits
        ):
            async with TestClient(TestServer(_make_app_with_override(state))) as client:
                resp = await client.post(
                    "/api/chat/slots/alias-a/advisor-override", json={"advisor_override": "off"}
        ...
        assert b.advisor_override == "off", "B's own commit on its new session stands"

    def test_every_decision_writer_bumps_the_mutation_token(self):
        """The three writers that represent a decision -- the endpoint's mirror
        onto the aliases, the post-save write onto an alias the reconciler
        created during the await, and a peer-acknowledged remote pick -- all go
        through ``set_advisor_override``; a restore from disk does not."""
        import inspect

        from kiro_crew.advisor.service import set_advisor_override
        from kiro_crew.dashboard import chat_handlers, chat_persistence

        slot = SimpleNamespace(advisor_override="inherit", _advisor_override_txn=3)
        set_advisor_override(slot, "off")
        assert (slot.advisor_override, slot._advisor_override_txn) == ("off", 4)
        endpoint = inspect.getsource(chat_handlers.api_chat_slot_advisor_override)
        assert endpoint.count("set_advisor_override(") == 2, "mirror loop + post-save alias write"
        assert "member.advisor_override = override" not in endpoint
        remote = inspect.getsource(chat_handlers._apply_remote_pick_locked)
        assert (
            'if control == "advisor_override":' in remote
            and "set_advisor_override(slot, value)" in remote
        )
        assert "set_advisor_override" not in inspect.getsource(
            chat_persistence.hydrate_advisor_meta
        )
