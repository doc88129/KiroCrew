...
class TestTheHookContractIsVersionGated:
    """The read gate relies on kiro-cli running ``preToolUse`` hooks for its
    builtin reads and blocking on exit 2, observed on 2.21.4. A release that
    stops honouring that would run the read ungated with nothing red to say
    why, so the floor is the probed release and an older or unparsable version
    makes the reviewer unavailable up front (the sandbox probe's shape)."""

    @pytest.mark.parametrize(
        "text, ok",
        [
            ("kiro-cli 2.21.4", True),
            ("kiro-cli 2.22.0", True),
            ("3.0.1", True),
            ("kiro-cli 2.21.3", False),
            ("kiro-cli 2.20.9", False),
            ("unavailable", False),
            ("", False),
            (None, False),
        ],
    )
    def test_floor(self, text, ok):
        assert read_gate.hook_contract_supported(text) is ok

    def test_an_unsupported_kiro_cli_makes_the_reviewer_unavailable(self, monkeypatch):
        ...
        service.sandbox_available = True
        service.read_gate_available = False
        cfg = SimpleNamespace(
            advisor=SimpleNamespace(enabled=True),
            agent=SimpleNamespace(acp_backend="kiro", role_models={}),
        )
        configure_from_config(cfg)
        assert service.reviewer_available is False
        assert service.enabled is False

    def test_the_probe_versions_the_binary_the_runtime_will_spawn(self, monkeypatch, tmp_path):
        """A ``kiro-cli`` on PATH is not what the reviewer runs when the operator
        pinned another binary (``KIROCREW_KIRO_BIN``): the probe must version the
        resolver's answer, the same one ``acp.client`` spawns."""
        import subprocess
        ...
        def fake_run(argv, **kw):
            calls.append(argv)
            return subprocess.CompletedProcess(argv, 0, stdout="kiro-cli 2.21.4\n", stderr="")

        monkeypatch.setattr(read_gate.subprocess, "run", fake_run)
        monkeypatch.setattr(
            read_gate, "resolve_kiro_cli", lambda **kw: str(tmp_path / "pinned" / "kiro-cli")
        )
        assert read_gate.probe_hook_contract() is True
        assert calls == [[str(tmp_path / "pinned" / "kiro-cli"), "--version"]]

    def test_no_resolvable_binary_is_unsupported(self, monkeypatch):
        monkeypatch.setattr(read_gate, "resolve_kiro_cli", lambda **kw: None)
        assert read_gate.probe_hook_contract() is False

    def test_unprobed_prerequisites_are_not_availability(self, monkeypatch):
        """``None`` means the startup probes have not answered yet; a live
        config PATCH landing in that window must not bind the reviewer on the
        ...
        monkeypatch.setattr("kiro_crew.advisor.service.get_advisor_service", lambda: service)
        service.sandbox_available = True
        service.read_gate_available = None
        cfg = SimpleNamespace(
            advisor=SimpleNamespace(enabled=True),
            agent=SimpleNamespace(acp_backend="kiro", role_models={}),
        )
