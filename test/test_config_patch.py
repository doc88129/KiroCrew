...
class TestAdvisorEnabled:
    ...
    def test_validator_reads_the_watcher_snapshot_not_the_disk(
        ...
        # Unanswered probes are not availability: refused with "still being probed".
        monkeypatch.setattr(get_advisor_service(), "sandbox_available", None)
        monkeypatch.setattr(get_advisor_service(), "read_gate_available", None)
        monkeypatch.setattr(get_advisor_service(), "reviewer_available", False)
        assert "probed" in (core._validate_advisor_enabled(True, SimpleNamespace()) or "")
        monkeypatch.setattr(get_advisor_service(), "sandbox_available", True)
        monkeypatch.setattr(get_advisor_service(), "read_gate_available", True)
        ...
        monkeypatch.setattr(get_advisor_service(), "read_gate_available", False)
        assert "2.21.4" in (core._validate_advisor_enabled(True, SimpleNamespace()) or "")
