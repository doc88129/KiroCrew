...
[#8117](https://github.com/kirodotdev/KiroCrew/pull/8117) fell through. It added
...
# shipped default, which is the tier a FRESH INSTALL runs and the one #8117
...
def _booted(fixture: str) -> Iterator[tuple[Any, _Client]]:
    ...
    into a later test in this interpreter.
    """
    try:
    ...
    os.environ["KIROCREW_KIRO_BIN"] = str(backend_path)
    try:
        with spawn_feature_gateway(fixture=fixture, approval="reads") as handle:
            yield handle, _Client(handle.port, handle.token)
    finally:
        if previous is None:
...
def test_seeded_sandbox_mode_boots_and_runs_a_turn(fixture: str) -> None:
    ...
    THE #8117 PIN. On Windows the ``auto`` tier MUST complete a turn: Kiro Crew
    ...
    precisely what shipped in #8117 and what
