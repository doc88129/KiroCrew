...
[PR #8117](https://github.com/kirodotdev/KiroCrew/pull/8117) added one conjunct
...
how #8117 stayed invisible. The prompt turn is the load-bearing assertion.
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        ...
        "POSIX host can represent without the platform mocks that let #8117 through"
    ),
)
...
class TestFreshWindowsHostPrecondition:
    ...
    def test_settings_probe_answers_false_on_this_host(self, fresh_windows_home):
        ...
        This is the value #8117 conjoined into the delegation predicate. Pinning
...
class TestWrapArgvContract:
    """The fail-closed pair, and the delegation branch #8117 broke.
    ...
    def test_classified_kiro_spawn_delegates_without_the_settings_file(self, fresh_windows_home):
        """THE #8117 PIN: delegation does not consult the settings probe.
        ...
        With #8117's conjunct restored this call raises ``SandboxUnavailableError``
...
def _write_fake_backend_launcher(directory: Path) -> Path:
    """A ``.cmd`` shim that runs the packaged fake ACP backend.

    ``KIROCREW_KIRO_BIN`` cannot point at ``fake_acp_backend.__file__`` on this
    platform the way ``test_e2e_smoke.py`` does: a ``.py`` path relies on a
    shebang, which Windows has no equivalent for, so ``CreateProcess`` would
    refuse it. ``CreateProcess`` DOES run a ``.cmd`` (it hands it to the command
    interpreter), and ``.cmd`` is in ``platform_compat``'s runnable-suffix set, so
    the candidate also survives ``is_executable_file``. ``build.yml`` stages its
    backend payload as ``kirocrew.cmd`` for the same reason.

    Named ``kiro-backend.cmd`` ON PURPOSE, not ``kiro-cli.exe``. Delegation here
    must come from the reviewed ACP call site passing ``is_kiro_cli=True``
    (``dashboard/handlers/sessions.py`` and ``handlers/agents.py``), never from the
    basename -- so a launcher whose name could not possibly satisfy
    ``_spawns_kiro_cli`` proves the classification is what grants it.
    """
    launcher = directory / "kiro-backend.cmd"
    launcher.write_text(
...
def fresh_windows_gateway(fresh_windows_home, tmp_path):
    ...
    ``docs/guides/windows-install.md`` promises and what #8117 falsified.
    """
...
class TestFreshWindowsGatewayDelegates:
    ...
    def test_health_then_one_session_and_one_prompt_succeed(self, fresh_windows_gateway):
        ...
        with #8117's conjunct in place health answers 200 while every ACP spawn
