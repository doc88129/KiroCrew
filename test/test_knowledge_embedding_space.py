...
guard cannot see a same-width change. Two halves close it:
``KnowledgeStore.reconcile_embedding_space`` invalidates the per-item signature,
and ``HybridRetriever._vector_search`` pins it.
"""
...
class TestReconcileEmbeddingSpace:
    def test_records_the_space_and_invalidates_active_signatures(self, store):
        item_id = _add_embedded(store, "Quorum design", _SIG_OLD)
        assert store.reconcile_embedding_space(_SIG_NEW) == 1
        assert store.recorded_embedding_space() == _SIG_NEW
        row = store.db.execute(
            "SELECT embedding, embedding_sig FROM items WHERE id = ?", (item_id,)
        ).fetchone()
        assert row["embedding_sig"] is None
        # The blob is untouched: the operation is O(1) per row and re-embeds
        # nothing, so re-stamping the same signature restores the prior state.
        assert row["embedding"] is not None

    def test_is_idempotent(self, store):
        _add_embedded(store, "Quorum design", _SIG_OLD)
        assert store.reconcile_embedding_space(_SIG_NEW) == 1
        # A matching signature is a no-op, so a caller may run this every boot.
        assert store.reconcile_embedding_space(_SIG_NEW) == 0
        assert store.reconcile_embedding_space(_SIG_NEW) == 0

    def test_re_stamping_the_old_signature_reverses_it(self, store):
        """Reversible by re-stamping, which is what makes it cheap to attempt."""
        item_id = _add_embedded(store, "Quorum design", _SIG_OLD)
        store.reconcile_embedding_space(_SIG_NEW)
        store.db.execute("UPDATE items SET embedding_sig = ? WHERE id = ?", (_SIG_OLD, item_id))
        store.db.commit()
        assert store.reconcile_embedding_space(_SIG_OLD) == 1
        assert store.recorded_embedding_space() == _SIG_OLD

    def test_leaves_non_active_items_alone(self, store):
        """Only active items are served, so only their signatures are the claim."""
        archived = _add_embedded(store, "Archived quorum", _SIG_OLD)
        store.db.execute("UPDATE items SET status = 'archived' WHERE id = ?", (archived,))
        store.db.commit()
        assert store.reconcile_embedding_space(_SIG_NEW) == 0
        row = store.db.execute(
            "SELECT embedding_sig FROM items WHERE id = ?", (archived,)
        ).fetchone()
        assert row["embedding_sig"] == _SIG_OLD

    def test_unrecorded_store_reports_none(self, store):
        assert store.recorded_embedding_space() is None

    def test_the_record_and_the_invalidation_are_one_transaction(self, store):
        """A failure between the two steps is a lie in either order.

        A stamped space over un-invalidated signatures serves stale vectors
        permanently -- the sig-gated rebuild would skip every row, because each
        one still reads as current. Invalidation without the stamp re-runs
        forever. So a failure must leave BOTH untouched, which only holds if they
        share a transaction.

        Failed in the DATABASE rather than by patching, because the connection's
        ``execute`` is read-only on pysqlite3 and a patched Python-side call
        cannot exercise a real rollback anyway: an ABORT trigger fires inside the
        open transaction exactly where a real conflict would.
        """
        item_id = _add_embedded(store, "Quorum design", _SIG_OLD)
        store.db.execute(
            "CREATE TRIGGER block_sig_update BEFORE UPDATE OF embedding_sig ON items "
            "BEGIN SELECT RAISE(ABORT, 'signature update blocked'); END"
        )
        try:
            with pytest.raises(Exception, match="signature update blocked"):
                store.reconcile_embedding_space(_SIG_NEW)
        finally:
            store.db.execute("DROP TRIGGER block_sig_update")

        assert store.recorded_embedding_space() is None
        row = store.db.execute(
            "SELECT embedding_sig FROM items WHERE id = ?", (item_id,)
        ).fetchone()
        assert row["embedding_sig"] == _SIG_OLD

        # And the store is not wedged: with the trigger gone the retry succeeds,
        # which is what makes an interrupted swap resumable rather than corrupt.
        assert store.reconcile_embedding_space(_SIG_NEW) == 1

    def test_reconcile_then_search_refuses_the_stale_vectors(self, store):
        """The two halves together: the invalidation is what the reader acts on."""
        _add_embedded(store, "Quorum replication ledger", _SIG_OLD)
        retriever = HybridRetriever(store, embedder=lambda q: list(_VEC), embed_sig=_SIG_OLD)
        assert len(retriever._vector_search("quorum") or []) == 1

        store.reconcile_embedding_space(_SIG_NEW)
        assert retriever._vector_search("quorum") == []
        # ...and the KB still answers, from FTS5 + graph.
        assert [r["title"] for r in retriever.search("quorum")] == ["Quorum replication ledger"]
