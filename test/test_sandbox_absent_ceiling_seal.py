...
class TestSealAppliesToAPreviouslyAbsentCeiling:
    ...
    def test_the_hidden_records_dir_is_materialised(self, crew_home):
        ...
        this test.
        """
        created = set(sandbox._materialize_maskable_dirs())
...
@_POSIX_ONLY
def _materialise_for(leaf: str) -> None:
    """Run whichever materialiser owns *leaf*.

    ``_CREW_NO_ALIAS_LEAVES`` spans both lists: ``panel-templates`` is a read-only
    ceiling and ``crew-panels`` is a masked directory, and each is created by its
    own function. Dispatching here keeps the refusal asserted for both without
    asserting it against a materialiser that never sees the leaf.
    """
    if leaf in sandbox._CREW_PRECREATE_HIDDEN_DIR_LEAVES:
...
class TestALinkedProtectedLeafRefusesTheSpawn:
    ...
    def test_a_real_directory_is_accepted(self, crew_home, leaf):
        ...
        _materialise_for(leaf)  # does not raise
...
class TestMaskableDirsAreMaterializedBeforeTheSpawn:
    ...
    def test_an_existing_directory_is_left_alone_and_not_reported(self, crew_home):
        # Every masked leaf, not just the first: a leaf this test did not create is
        # created by the call and reported, which says nothing about the property.
        for leaf in sandbox._CREW_PRECREATE_HIDDEN_DIR_LEAVES:
