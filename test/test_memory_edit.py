...
def test_paged_email_extraction_and_atomic_batch(store):
    seed(store)
    assert (
        store.set_semantic(
            "user.mailpolicy", "Archive 邮件 after thirty days.", 1.0, "user_explicit"
        )
        is None
    )
    first = memory_edit.list_records(store, {"topic": "email"}, limit=50)
    second = memory_edit.list_records(store, {"topic": "email"}, offset=50)
...
def test_explicit_cross_page_exclusions_and_single_correct(store):
    seed(store)
    entries = memory_edit.list_records(store, {"topic": "email"}, limit=100)["entries"]
    excluded = [{"kind": entries[0]["kind"], "id": entries[0]["id"]}]
    preview = memory_edit.preview_edit(
        store,
        "chosen",
        b"secret",
        body(selection={"query": {"topic": "email"}, "exclude": excluded}),
    )
...
def test_forget_episode_and_semantic_is_reversible_history(store):
    ...
    memory_edit.apply_edit(store, "chosen", b"secret", preview["preview_id"])
    assert memory_edit.list_records(store, {"topic": "email"})["total"] == 0
    assert (
        store.db.execute(
...
def test_query_refresh_enforces_the_same_selection_caps_without_writes(store, monkeypatch, cap):
    ...
    with pytest.raises(memory_edit.MemoryEditError) as error:
        memory_edit.refresh_records(store, selection={"query": {"topic": "email"}})
...
def test_conflict_history_and_owner_resolution_preserve_evidence(store):
    seed(store, 1)
    assert (
    ...
    assert record["metadata"]["pending_conflicts"] == 1
    history = memory_edit.record_history(store, {"kind": "fact", "id": record["id"]})
    proposal = next(row for row in history["entries"] if row["status"] == "conflict")
    assert "proposed@elsewhere.example" in proposal["after_json"]
    assert proposal["base_revision"] == history["current_revision"]
    preview = memory_edit.preview_edit(
        store,
        "chosen",
        b"secret",
    ...
    history = memory_edit.record_history(store, {"kind": "fact", "id": record["id"]})
    assert any(row["id"] == proposal["id"] for row in history["entries"])


def test_keep_current_value_resolves_proposals_without_mutating_content(store):
    seed(store, 1)
...
def test_new_proposal_invalidates_keep_current_preview(store):
    seed(store, 1)
    store.set_semantic("user.email000", "wrong@elsewhere.example", 0.9, "consolidation")
    record = memory_edit.list_records(store, {"q": "email000"})["entries"][0]
    preview = memory_edit.preview_edit(
    ...
    assert error.value.status == 409
    current = memory_edit.list_records(store, {"q": "email000"})["entries"][0]
    assert current["metadata"]["pending_conflicts"] == 2
