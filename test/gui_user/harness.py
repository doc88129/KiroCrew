...
Tool shape: the native Bedrock computer-use tool (``computer_20251124`` behind
the ``computer-use-2025-11-24`` beta) when the model accepts it, else a plain
...
- Stay inside the browser window. Do not open other applications, do not change system settings, do not navigate to any site other than the one already open.
...
def custom_tools() -> list[dict[str, Any]]:
    ...
    return [
        ...
        tool(
            "key",
            "Press one key or chord in xdotool syntax: Return, Escape, Tab, BackSpace, ctrl+l, ctrl+a, Page_Down.",
