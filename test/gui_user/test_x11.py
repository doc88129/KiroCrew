...
class TestDisplayGuard:
    ...
    @pytest.mark.parametrize("display", [":99", ":10", ":2", "localhost:99.0"])
...
class TestBuildArgv:
    ...
    def test_key_normalizes(self) -> None:
        ...
        # The native tool spells it `text`; the custom fallback used to say `key`.
...
class TestDisplay:
    ...
    def test_xdotool_failure_surfaces_as_x11error(self, tmp_path: Path) -> None:
        ...
        d = x11.Display(
            ":99", GEO, tmp_path, runner=runner, grabber=lambda _d: _FakeImage((1600, 1000))
        )
