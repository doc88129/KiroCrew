...
def scenario_line(sc: dict[str, Any]) -> str:
    ...
    if last.get("error"):
        detail = f"{detail}: {str(last['error'])[:80]}"
    return (
...
def _final_text_blocks(summary: dict[str, Any], *, max_chars: int = 1200) -> list[str]:
    out: list[str] = []
    for sc in summary.get("scenarios", []):
        ...
        if sc.get("status") == "PASS" and text:
            ...
            if issues and not issues[0].strip().lower().endswith("none"):
                out.append(
                    f"<details><summary><code>{sc['name']}</code> — UI issues noticed</summary>\n\n"
                    f"{issues[0].strip()}\n\n</details>"
                )
            continue
        if text or last.get("error"):
            body = text[:max_chars] + ("…" if len(text) > max_chars else "")
            if last.get("error"):
                body = f"Error: {last['error']}\n\n{body}".strip()
            out.append(
                f"<details><summary><code>{sc['name']}</code> — final report</summary>\n\n"
                f"```\n{body}\n```\n\n</details>"
            )
    return out
