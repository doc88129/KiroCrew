...
class TestGuardDedupe:
    def test_duplicate_note_is_suppressed_within_epoch(self):
        guard = EmissionGuard()
        update = guard.begin_update()
        note = AdvisorNote(severity="concern", text="unbounded retry loop")
        assert guard.admit(note, update) is not None
        assert guard.admit(note, update) is None

    def test_normalization_catches_whitespace_and_case_variants(self):
        guard = EmissionGuard()
        update = guard.begin_update()
        guard.admit(AdvisorNote(severity="nit", text="Missing  Null Check"), update)
        assert guard.admit(AdvisorNote(severity="nit", text="missing null check"), update) is None

    def test_content_free_note_never_emits(self):
        guard = EmissionGuard()
        update = guard.begin_update()
        for text in ("", "   ", "\n\t", "...", "!!"):
            assert guard.admit(AdvisorNote(severity="nit", text=text), update) is None

    def test_epoch_reset_clears_dedupe(self):
        guard = EmissionGuard()
        update = guard.begin_update()
        note = AdvisorNote(severity="concern", text="same finding")
        guard.admit(note, update)
        guard.begin_epoch()
        assert guard.admit(note, update) is not None
...
class TestGuardBudget:
    def test_non_blocker_budget_is_enforced_per_update(self):
        ...
        update = guard.begin_update()
        admitted = [
            guard.admit(AdvisorNote(severity="nit", text=f"note {i}"), update) for i in range(4)
        ]
    ...
    def test_blockers_are_exempt_from_budget(self):
        ...
        update = guard.begin_update()
        guard.admit(AdvisorNote(severity="nit", text="first"), update)
        assert guard.admit(AdvisorNote(severity="nit", text="second"), update) is None
        assert guard.admit(AdvisorNote(severity="blocker", text="real problem"), update) is not None
    ...
    def test_budget_resets_on_new_update(self):
        ...
        guard.admit(AdvisorNote(severity="nit", text="first"), update)
        update = guard.begin_update()
        assert guard.admit(AdvisorNote(severity="nit", text="fresh note"), update) is not None
...
class TestGuardEscalation:
    def test_higher_severity_duplicate_replaces_lower(self):
        guard = EmissionGuard()
        update = guard.begin_update()
        guard.admit(AdvisorNote(severity="nit", text="race in cache write"), update)
        escalated = guard.admit(AdvisorNote(severity="blocker", text="race in cache write"), update)
    ...
    def test_lower_severity_duplicate_stays_suppressed(self):
        guard = EmissionGuard()
        update = guard.begin_update()
        guard.admit(AdvisorNote(severity="blocker", text="race in cache write"), update)
        assert guard.admit(AdvisorNote(severity="nit", text="race in cache write"), update) is None

    def test_non_blocker_escalation_spends_the_update_budget(self):
        # A nit re-emitted as a concern is a second non-blocker card; only
        # blockers are exempt from the budget.
        guard = EmissionGuard(non_blocker_budget=2)
        update = guard.begin_update()
        assert guard.admit(AdvisorNote(severity="nit", text="race in cache write"), update)
        assert guard.admit(AdvisorNote(severity="concern", text="race in cache write"), update)
        assert update.used == 2
        assert guard.admit(AdvisorNote(severity="nit", text="unrelated fresh nit"), update) is None

    def test_non_blocker_escalation_honours_the_cooldown(self):
        clock = [0.0]
        ...
        assert (
            guard.admit(AdvisorNote(severity="concern", text="race in cache write"), update) is None
        )
        clock[0] = 61.0
        assert guard.admit(AdvisorNote(severity="concern", text="race in cache write"), update)

    def test_blocker_escalation_is_exempt_from_budget_and_cooldown(self):
        clock = [0.0]
        guard = EmissionGuard(non_blocker_budget=1, cooldown_secs=60, clock=lambda: clock[0])
        update = guard.begin_update()
        assert guard.admit(AdvisorNote(severity="nit", text="race in cache write"), update)
        guard.note_interruption()
        assert guard.admit(AdvisorNote(severity="blocker", text="race in cache write"), update)
        assert update.used == 1
...
class TestGuardCooldown:
    def test_cooldown_after_blocker_suppresses_non_blockers(self):
        ...
        guard = EmissionGuard(cooldown_secs=60, clock=lambda: clock[0])
        update = guard.begin_update()
        guard.admit(AdvisorNote(severity="blocker", text="stop now"), update)
        guard.note_interruption()
        clock[0] += 10
        assert guard.admit(AdvisorNote(severity="nit", text="minor thing"), update) is None

    def test_new_blocker_is_exempt_from_cooldown(self):
        clock = [100.0]
        guard = EmissionGuard(cooldown_secs=60, clock=lambda: clock[0])
        update = guard.begin_update()
        guard.admit(AdvisorNote(severity="blocker", text="first blocker"), update)
        guard.note_interruption()
        clock[0] += 10
        assert (
            guard.admit(AdvisorNote(severity="blocker", text="second blocker"), update) is not None
        )
    ...
    def test_cooldown_expires(self):
        ...
        guard = EmissionGuard(cooldown_secs=60, clock=lambda: clock[0])
        update = guard.begin_update()
        guard.admit(AdvisorNote(severity="blocker", text="stop"), update)
        guard.note_interruption()
        clock[0] += 61
        assert guard.admit(AdvisorNote(severity="nit", text="minor"), update) is not None
...
class TestInterruptionCapPerEpoch:
    ...
    def test_a_reservation_counts_before_the_steer_lands(self):
        # Two blocker deliveries can overlap on the event loop (each awaits the
        # steer); checking the cap and counting only after the steer lands lets
        # both pass a cap of one. Reserving takes the slot at the check.
        from kiro_crew.advisor.guard import EmissionGuard

        guard = EmissionGuard(interruption_cap=1)
        assert guard.reserve_interruption() is True
        assert guard.may_interrupt() is False
        assert guard.reserve_interruption() is False

    def test_a_released_reservation_reopens_the_slot(self):
        from kiro_crew.advisor.guard import EmissionGuard

        guard = EmissionGuard(interruption_cap=1)
        assert guard.reserve_interruption() is True
        guard.release_interruption()
        assert guard.may_interrupt() is True
        assert guard.reserve_interruption() is True

    def test_confirming_a_reservation_does_not_double_count(self):
        from kiro_crew.advisor.guard import EmissionGuard

        guard = EmissionGuard(interruption_cap=2)
        assert guard.reserve_interruption() is True
        guard.note_interruption(reserved=True)
        assert guard.may_interrupt() is True, "one steer must consume exactly one slot"
        assert guard._in_cooldown() is True
