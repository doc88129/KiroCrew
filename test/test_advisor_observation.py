...
class TestSealedEpoch:
    """A hard kill seals observation until the next turn attaches: a late
    record from the runner being torn down is dropped, nothing completes, and
    ``begin_turn`` (the next attach) reopens observation."""

    def test_records_and_completion_are_dropped_while_sealed(self):
        from kiro_crew.advisor.observation import AdvisorObserver

        obs = AdvisorObserver(parent_session_key="dashboard:a", turn_id="t1")
        obs.record_segment("before the kill")
        obs.seal()
        obs.record_tool_result("fs_read", "late result from the killed runner")
        obs.record_segment("late text")
        assert obs.drain_update() is None
        assert obs.complete(stop_reason="end_turn") is None
        assert obs.take_completed() is None
        obs.begin_turn()
        obs.record_segment("next turn")
        update = obs.drain_update()
        assert update is not None and update.segments == ["next turn"]
