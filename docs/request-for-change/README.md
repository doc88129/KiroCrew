...
|---|---|---|
| [rfc-central-governance-ceiling.md](rfc-central-governance-ceiling.md) | `draft` | Partial. On this branch (#7362): the tighten-only ladder in `load_security_policy` (`platform/governance.py`), central-above-env precedence, Ed25519 policy signing, and a machine-pinned managed rung read by `_read_managed_policy` / `_managed_policy_path` on Linux (`/etc/kirocrew-managed/security_policy.json`) and macOS (Computer-Level profile at `/Library/Managed Preferences/dev.kirocrew.plist`), both root-owned and ACL-checked. Not on any branch: the withdrawn `break_glass` override and its four defect classes (#9106), the Windows `HKLM\SOFTWARE\Policies\<vendor>\<product>` rung (Windows stays advisory), a user-level-profile rung, and the parent-authenticated carrier that would let a sandboxed child adopt the managed document (#9140) |
