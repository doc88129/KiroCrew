...
- **Host-floor patches use `_host_floor_patch`, never the test's shared
  ...
  the floor without removing it. `_host_floor_patch` drains the shared stack
  before restoring its own pins at teardown; reversing that order can reinstall
  a stale per-test path or delete a restored inherited environment value. Tests
