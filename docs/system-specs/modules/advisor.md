...
source tree, so the module's verdicts are proven, not assumed). The contract
itself is version-gated: startup versions the kiro-cli the runtime's own
resolver will spawn (so an operator's `KIROCREW_KIRO_BIN` pin is what gets
versioned), and a release older than the one the hook behaviour was observed
on (`read_gate.MIN_KIRO_CLI_VERSION`, 2.21.4), or no readable version, composes
into `reviewer_available` as unavailable, refused up front with the reason like
the sandbox probe. Both probes must have answered yes -- an unanswered probe is
not availability, so a live settings change landing before the probes finish
refuses with "still being probed", and the startup task re-applies the newest
live snapshot once the probes are in. Under that gate the OS sandbox is the
