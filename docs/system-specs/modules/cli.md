...
| `kirocrew memory backup/backups/restore` | Take a hot copy of every declared store now (`--keep <n>`), list a store's copies newest-first (`--store`), or restore one (`--store`, `--from <file>`, defaulting to that store's newest). Private V2 restoration stages for gateway startup; `restore --store <member-store> --cancel-pending` cancels a staged restore while preserving active memory and its backup, and is mutually exclusive with `--from`. All three dispatch BEFORE the shared vector store is opened, because opening it raises on exactly the corrupt file these verbs recover. See [memory-skills-hooks](memory-skills-hooks.md#automatic-backups-memory_backuppy) |
