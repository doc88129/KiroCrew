...
dashboard memory management stays available. Global V1 retains its own data and
physical schema and existing prompt retrieval. Optional tool-driven recall,
...
First private preparation pins an immutable session/store record under the
read-only binding root; metadata downgrade or reassignment fails after restart.
...
owner preference/project GET routes expose that failure as HTTP 503. A rebuild
...
lineage keeps its existing schema. A future explicit V1 migration is deferred
until private V2 is tuned. Initializing an existing member provisions a separate
...
detectable instead of silently serving another crew. These rows record store
identity; the private ownership manifest and config separately bind exactly one
member to that store. Structure stays authoritative: `detect_lineage` reads
...
Global Memory is **V1**. Every configured member except the reserved `default`
alias owns a unique **V2** store, provisioned empty before member creation is
published. The store's owner and algorithm version are recorded in config and
its protected manifest. Private stores cannot be shared or rebound. Existing
members without V2 use an explicit Initialize action; their old data is retained
and is not automatically imported. Ownership validation is specified in
...
| Member DM, without Crew Mode | The selected member's V2 preferences, project notes, task-relevant facts, episodes and rules |
| Crew Mode | The coordinator keeps its own binding; each named delegate receives that delegate's V2 store. A handoff shares its explicit task and result, not direct access to another member's store |
...
checked-in `eval/bench/data/member-v2-hybrid-qwen3.json` is a historical
V2 snapshot before the retention-policy change and records the corpus/model SHA-256, policy revision,
...
states that measured values and original source seals are unchanged. The separate
`member-v2-hybrid-qwen3-current.json` records the rerun after the retention change;
run metadata and source hashes distinguish snapshots without creating product
subversions. With the
...
Chinese/Japanese/Korean fact and episode fixtures, oversized-row skipping and
forgotten-row exclusion passed. This is corpus-informed retrieval evidence,
...
a failed switch. An external journal allows startup to finish an interrupted
switch or recognize one completed before journal cleanup. Unrecoverable
activation refuses startup with an actionable error; it never serves global
memory or an incomplete restore. A second pending restore is refused so it
...
After cancellation or completed startup, GET explicitly returns false/false/null.
...
- **Retention**: `memory.backup_keep` (default 7), clamped to at least 1. A retention
  ...
  across reload/save, so disabling the heartbeat pass or extending recovery
  retention survives a gateway restart.
...
activation.
Activation failure aborts startup, retaining the journal, stage and prior data.
...
The default is one native thread and configured counts are capped at two,
including bulk overrides. At most eight pending native jobs are retained, with
...
|---|---|---|
| no `?store=` | the global store, preserving the existing dashboard default | none added |
...
absent parameter always answers from the global store; an unverified `X-Session-Key`
...
The global and member editors share `MemoryRecordsEditor`. Their paged list
uses `GET /api/memory/records` with `store`, `q`, `kind`, `topic`, `offset` and
`limit` (1–100). Filtering precedes pagination. `topic=email` combines cached
addresses with deterministic email-related terms, including Chinese; it is
a review filter, not an assertion of exhaustive semantic extraction. Search
...
discarding already loaded history. A conflict is pending only while its base revision equals the current
