...
| Decision | `partial` | `decide_monitor` in `monitoring/decision.py` is a pure function with budgets and terminal handling, but edge-triggered and with no coalescing |
...
`format_monitor_wake` does out of `MonitorObservation.canonical` and
`MonitorState.wake_instructions`.
...
The package has no base class: zero `ABC`, zero `abstractmethod`, and no
behaviour inheritance anywhere in `monitoring/`.

All inheritance in the package is enums on `str, Enum` -- `MonitorDecision`,
`MonitorObservationStatus`, `ProviderErrorKind`, `MonitorOutcome`,
`MonitorActionDisposition`, `MonitorDispatchResult` -- plus one exception,
`ShadowWakeDeliveryRefused` on `RuntimeError`. All polymorphism is Protocols, and
every one but `GitHubShadowProvider` in `shadow.py` is private: `_Loop`,
`_Service` and `_Provider` in `controller.py`. Everything else, the dataclasses in
`models.py` and `github_pull_request.py` together with the plain classes
`MonitorController` and `GitHubPullRequestProvider`, has no base at all.

This is not a style note. It is the precise statement that a second kind has
nothing to subclass and no hook to implement. The extension point does not exist
yet, so the work described here is to create one, not to conform to one.
...
lost to exactly the long-running compaction it exists to survive. Today no engine
state holds it: the only consecutive counter in the package is
`consecutive_provider_errors`, which counts provider failures rather than
identical verdicts.
...
- Superseded attempts collapse to the newest per check identity. A host keeps
  ...
  identity in `collapse_superseded`, while the structured provider gives every
  check row its own group key in `_normalize_checks` and maps `CANCELLED` to
  failed in `_normalize_check`, so it can wake on a phantom failure.
