...
export const api = {
  ...
  cancelMemberMemoryRestore: (store: string) =>
    post('/api/memory/restore/cancel', { store }).then(j) as Promise<{
      ok: boolean; cancelled: boolean; pending: false; restart_required: false; pending_restore: null
    }>,
  ...
  memoryRecords: (store: string, query: MemoryRecordQuery, offset = 0, limit = 50) =>
    fetch('/api/memory/records?' + new URLSearchParams({ store: store || 'default', q: query.q, kind: query.kind, ...(query.topic ? { topic: query.topic } : {}), offset: String(offset), limit: String(limit) })).then(j) as Promise<{ entries: MemoryRecord[]; total: number; has_more: boolean }>,
