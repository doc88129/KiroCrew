...
// Stubbed because the real one talks to the folder API. What matters at this level is
// WHICH sessions get filed and under what name; `sessionFolder.test.ts` owns how.
const fileSessionInCommandFolder = vi.fn(async () => 'folder-1')
vi.mock('./sessionFolder', () => ({
...
describe('CommandBarOverlay contributed commands', () => {
  ...
  it('files an argument-taking command under its title too', async () => {
    ...
    fireEvent.keyDown(input, { key: 'Enter' })
    await waitFor(() => expect(fileSessionInCommandFolder).toHaveBeenCalled())
    expect(fileSessionInCommandFolder.mock.calls[0]?.[2]).toBe('Approve all PRs')
  })
