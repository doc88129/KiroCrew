...
function SeedMemory({ store, onClose, onCopied }: { store: string; onClose: () => void; onCopied: () => void }) {
  ...
  return <Modal open title={t('memoryV2.copy_dialog_title')} guardAccidentalDismiss={selection.length > 0} onClose={copy.isPending ? () => {} : onClose} footer={<div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="flex min-w-0 flex-col gap-3">
      ...
      <Input aria-label={t('memoryV2.search')} placeholder={t('memoryV2.search')} value={filter} onChange={e => setFilter(e.target.value)} />
      {/* No hand-off: the selection is an unsaved copy request. */}
      <ErrorNotice message={errorText(data.error || copy.error) || (copy.data?.partial ? t('memoryV2.copy_partial') : '')} />
      ...
      {copy.data && <div className="text-[13px]">
        ...
        {copy.data.results.filter(r => r.reason).map((r, i) => <p key={`${r.id}:${i}`}>{r.reason}</p>)}
      </div>}
    </div>
  </Modal>
}
...
export default function MemberMemoryPanel({ store, summary, onDirtyChange }: { store: string; summary: MemoryStoreSummary; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  const privateMemory = summary.memory_version === 2
  ...
  const empty = total === 0
  const copyAction = privateMemory && <Btn primary={empty} className="min-h-11 justify-center" onClick={() => setCopying(true)} disabled={!summary.exists}><Copy className="lucide-inline" />{t('memoryV2.copy_title')}</Btn>
  ...
  return <div className="min-w-0 space-y-4">
    <Card className="mb-0 min-w-0 py-4">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0 space-y-2"><CardTitle className="mb-0 min-w-0 text-lg leading-snug"><span className="min-w-0 break-words">{t('memoryV2.title', { member: summary.owner_member || summary.name })}</span></CardTitle><div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[12px] text-muted">{privateMemory && <><Badge variant="muted">{PRIVATE_MEMORY_VERSION}</Badge><span>{t('memoryV2.private_status')}</span></>}{total !== undefined && <span>{t('memoryV2.remembered_count', { count: fmtNumber(total) })}</span>}</div></div>
        </div>
        {!empty && <div className="shrink-0">{copyAction}</div>}
      </div>
    </Card>
    {!privateMemory && <p className="text-[13px] text-muted">{t('pages.kiroCrewAgentsPage.private_memory_legacy')}</p>}
    <Tabs value={section} layoutId={`member-memory-sections-${store}`} onValueChange={selectSection}>
      ...
      <TabsContent value="recovery" forceMount hidden={section !== 'recovery'} className="space-y-4">{visited.has('recovery') && <><p className="text-[13px] text-muted">{t('memoryV2.recovery_hint')}</p><MemoryBackupsCard store={store} privateMemory={privateMemory} /><MemoryRetiredCard store={store} /><details onToggle={event => setAdvanced(event.currentTarget.open)}><summary className="cursor-pointer rounded-lg border border-border p-3 text-[13px] text-muted"><SlidersHorizontal className="lucide-inline mr-2" />{t('memoryV2.advanced')}</summary>{advanced && <div className="mt-3"><MemoryCarveCard store={store} /></div>}</details></>}</TabsContent>
    </Tabs>
    {copying && <SeedMemory store={store} onClose={() => setCopying(false)} onCopied={refresh} />}
  </div>
}
