 ...
 *  catalogs interpolate `{{count}}` as a plain number (no digit localization),
 *  so the first occurrence of the digits is the number being qualified. */
export function floorCountText(text: string, count: number): string {
  const digits = String(count)
  const at = text.indexOf(digits)
  return at < 0 ? text : text.slice(0, at + digits.length) + '+' + text.slice(at + digits.length)
}
