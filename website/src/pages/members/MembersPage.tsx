...
import { ArrowLeft, ChevronRight, Circle, Clock, ExternalLink, Goal, Pause, Pencil, Route, Star, UserPlus, Users, Webhook } from 'lucide-react'
...
/** `2 chats` -> `2+ chats` when the day's count is a floor (see ActivityDay.isFloor). */
const floorCount = (text: string, n: number, isFloor: boolean) => (isFloor ? floorCountText(text, n) : text)
                    ...
                    <li key={day.dayStart} className="contents">
                      ...
                      >
                        ...
                        <span className="min-w-0 truncate">
                          ...
                          {day.chats > 0 && floorCount(t('pages.membersPage.activity_chat_count', { count: day.chats }), day.chats, day.isFloor)}
                          {day.chats > 0 && day.routed > 0 && PROJECT_SEPARATOR}
                          {day.routed > 0 && floorCount(t('pages.membersPage.activity_routed_count', { count: day.routed }), day.routed, day.isFloor)}
                          {first && (
                      ...
                      {/* Plain text, not pills: these open nothing, and a bordered
                          chip reads as a button. A routed pick carries an icon as
                          well as the accent colour, so it is told apart without
                          colour or hover. */}
                      {open && (
                        <ul
                        ...
                        >
                          {day.entries.map((e, i) => {
                            ...
                            return (
                              <li
                                key={`${e.ts}-${i}`}
                                className={`inline-flex items-center gap-0.5 font-mono text-[10px] leading-4 ${routed ? 'text-accent' : 'text-muted'}`}
                                title={
                                  (routed
