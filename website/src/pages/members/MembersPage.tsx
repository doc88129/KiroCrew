    ...
    <div className="flex h-full min-h-0 pb-2" data-testid="members-page">
      ...
      >
        ...
        {/* The at-rest marker that the list is narrowed: ONE aggregate chip in
            ...
            AUTOSDE max-two-buttons-per-row; the sidebar's tag filter takes the
            same aggregate form for the same reason. The search text is not in
            it: it is visible in the field, with its own clear button. */}
        {narrowed && (
          <div className={FILTER_CHIP_ROW_CLS} data-testid="member-filter-chips">
            <FilterChip
              label={fmtList(activeFilterLabels, { type: 'unit', style: 'short' })}
              color="var(--accent)"
