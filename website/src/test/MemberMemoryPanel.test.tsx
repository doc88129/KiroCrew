...
const { api } = vi.hoisted(() => ({ api: {
  memoryStores: vi.fn(), memoryRecords: vi.fn(), memoryEditPreview: vi.fn(), memoryEditApply: vi.fn(), memoryRecordsRefresh: vi.fn(), memoryRecordHistory: vi.fn(), memberMemoryPage: vi.fn(), memorySeed: vi.fn(), memoryRecall: vi.fn(),
...
beforeEach(() => {
  ...
  api.memoryEditPreview.mockResolvedValue({ preview_id: 'reviewed-preview', expires_at: '2026-12-31T12:00:00Z', matched_count: 1, changed_count: 1, unchanged_count: 0, entries: [], preview_has_more: false, warnings: [] })
...
describe('private member memory lifecycle', () => {
  it('waits for the owning identity without showing a directory name or offering writes', async () => {
    ...
    expect(screen.queryByRole('button', { name: 'Choose starting knowledge' })).toBeNull()
  ...
  it.each(['absent', 'ownerless'])('refuses a %s private identity instead of presenting the store id', async condition => {
    const catalog = condition === 'absent' ? stores.filter(s => s.name !== MEMBER_STORE) : stores.map(s => s.name === MEMBER_STORE ? { ...s, owner_member: '' } : s)
  ...
  it('corrects an experience under its stable id and retains the draft through a failed save', async () => {
    ...
    fireEvent.change(within(dialog).getByRole('textbox', { name: 'Correct' }), { target: { value: text } })
    ...
    expect(within(dialog).getByRole('textbox', { name: 'Correct' })).toHaveValue(text)
    fireEvent.click(within(dialog).getByRole('button', { name: 'Preview changes' }))
  ...
  it('keeps persisted pending restore visible after remount and blocks duplicate staging', async () => {
    ...
    for (let visit = 0; visit < 2; visit++) {
      fireEvent.mouseDown(screen.getByRole('tab', { name: 'Recovery' }), { button: 0, ctrlKey: false })
      expect(await screen.findByText(/Restore is ready. Restart the gateway/)).toBeInTheDocument()
  ...
  it('cancels a staged member restore with explicit success and keeps the original backup available', async () => {
    ...
    expect(await screen.findByText('Staged restore cancelled. Current memory and the backup are unchanged.')).toBeInTheDocument()
    expect(api.cancelMemberMemoryRestore).toHaveBeenCalledWith(MEMBER_STORE)
    expect(screen.getByRole('button', { name: 'Restore', exact: true })).toBeEnabled()
    expect(api.memoryRestoreBackup).not.toHaveBeenCalled()
  })
  ...
  it('shows the source member avatar while copying without replacing the target identity', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Choose starting knowledge' }))
    const dialog = screen.getByRole('dialog', { name: 'Copy knowledge' })
  ...
  it('corrects and forgets the selected member items after explicit confirmation', async () => {
    ...
    let dialog = screen.getByRole('dialog', { name: 'Correct memory' })
    fireEvent.change(within(dialog).getByRole('textbox', { name: 'Correct' }), { target: { value: 'Review every changed boundary' } })
    fireEvent.click(within(dialog).getByRole('button', { name: 'Preview changes' }))
  ...
  it('corrects a directive rule while retaining its structured metadata', async () => {
    ...
    const dialog = screen.getByRole('dialog', { name: 'Correct memory' })
    fireEvent.change(within(dialog).getByRole('textbox', { name: 'Correct' }), { target: { value: 'New review rule' } })
    fireEvent.click(within(dialog).getByRole('button', { name: 'Preview changes' }))
  ...
  it('validates a structured fact correction before sending a parsed object', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Correct' }))
    const dialog = screen.getByRole('dialog', { name: 'Correct memory' })
    const editor = within(dialog).getByRole('textbox', { name: 'Correct' })
  ...
  it('keeps an unconfirmed partial copy distinct from a skipped or successful copy', async () => {
    ...
    expect(within(dialog).getByText('Could not confirm the database write')).toBeInTheDocument()
  ...
  it.each(['member', 'copy'] as const)('preserves server-normalized text matches in the %s search', async surface => {
    ...
    const view = surface === 'copy' ? within(screen.getByRole('dialog', { name: 'Copy knowledge' })) : screen
