...
describe('query-string builders', () => {
  it('memory records preserve the chosen store, filter and page without leaking query delimiters', async () => {
    await api.memoryRecords('member-reviewer', { q: 'owner+team@example.com & release', kind: 'fact', topic: 'email' }, 50, 25)
    const query = new URL(call().url, 'http://localhost').searchParams
    expect(Object.fromEntries(query)).toEqual({ store: 'member-reviewer', q: 'owner+team@example.com & release', kind: 'fact', topic: 'email', offset: '50', limit: '25' })
