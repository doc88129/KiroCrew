...
describe('AdvisorOverrideControl', () => {
  ...
  it('explains the feature for the untouched default without claiming a state', async () => {
    ...
    expect(screen.queryByText('Not reviewing. Advice saved for your next turn is dropped.')).not.toBeInTheDocument()
    ...
    await screen.findByText('Not reviewing. Advice saved for your next turn is dropped.')
  })
  ...
  it('lives in the same per-session model panel as reasoning effort', async () => {
    ...
    expect(screen.getByText('Not reviewing. Advice saved for your next turn is dropped.')).toBeInTheDocument()
  })

  it('keeps arrow keys on the select instead of handing them to the model list', async () => {
    // The select sits under the picker's container onKeyDown, whose listbox
    // handler preventDefaults ArrowUp/Down/Home/End and moves focus into the
    // model list. Those keys must stay with the native control so its
    // collapsed-state value cycling and focus survive.
    ...
    const select = await screen.findByRole('combobox', { name: 'Advisor mode' })
    select.focus()
    for (const key of ['ArrowDown', 'ArrowUp', 'Home', 'End']) fireEvent.keyDown(select, { key })
