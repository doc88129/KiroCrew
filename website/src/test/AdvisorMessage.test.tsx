...
describe('AdvisorMessage', () => {
  ...
  it('labels advice the user dropped by turning the Advisor off', () => {
    // The backend flips a preserved card to this state when the session's
    // override is set to off before the next turn consumed the advice.
    render(<AdvisorMessage message={message('concern', 'preserved', { advisorState: 'dropped_by_opt_out' })} />)

    expect(screen.getByText('Dropped when you turned the Advisor off')).toBeInTheDocument()
    expect(screen.queryByText('Saved for next turn')).not.toBeInTheDocument()
  })
