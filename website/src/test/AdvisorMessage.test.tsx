...
describe('AdvisorMessage', () => {
  ...
  it('cleans legacy content when structured display fields are absent', () => {
    const legacy = message('concern', 'steered', { advisorText: undefined, advisorEvidence: undefined })
    render(<AdvisorMessage message={legacy} />)

    expect(screen.getByText('Raw fallback finding.')).toBeInTheDocument()
    expect(screen.queryByText(preamble)).not.toBeInTheDocument()
    expect(screen.queryByText(/\[concern\]/)).not.toBeInTheDocument()
    expect(screen.getByText('Steered')).toBeInTheDocument()

    fireEvent.click(screen.getByRole('button', { name: 'Evidence' }))
    expect(screen.getByText('Raw fallback evidence.')).toBeInTheDocument()
  })
