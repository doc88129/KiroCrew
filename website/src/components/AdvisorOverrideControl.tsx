...
export default function AdvisorOverrideControl({
  ...
  // The model picker's container handler treats these keys as list navigation
  // (preventDefault + focus into the model list); the native select owns them
  // while it has focus.
  const keepListNavigationKeys = (event: KeyboardEvent<HTMLSelectElement>) => {
    switch (event.key) {
      case 'ArrowDown':
      case 'ArrowUp':
      case 'Home':
    ...
    <div className="shrink-0 border-t border-border px-3 py-2">
      <div className="flex items-center justify-between gap-3">
        ...
        <NativeSelect
          ...
          onKeyDown={keepListNavigationKeys}
      ...
      {mutation.isError && (
        <div className="mt-1.5">
          ...
          <ErrorNotice
            message={
              // The backend refusal gets a localized sentence; any other failure
              // shows the plain line and its raw body goes to the console.
