    ...
    const onMessage = (e: MessageEvent) => {
      ...
      if (data.type === 'mc-unread-slots') {
        ...
        if (stage === 'script-error') {
          ...
          // in its graph failed to FETCH. The one cause a reload cannot fix by
          // itself is a 404 the desktop's HTTP cache is replaying under this
          // origin, so evict that origin's cache and reload once. ONCE: a 404
          // the gateway is really serving right now (dist/ mid-swap) would
          // otherwise loop clear/reload forever; after the one attempt the load
          // watchdog surfaces the error panel, and Retry re-opens the budget.
