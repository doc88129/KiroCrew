...
/** One active filter as a dismissible pill in the filter's own colour; the
 *  click clears exactly that filter. `label` is the caller's — the sidebar
 *  appends its window and count, the roster its count — so one recipe carries
 *  whatever a list has to say about the filter without knowing what it is. */
export function FilterChip({ label, color, clearLabel, onClear, testId }: {
  label: string
  /** The filter's colour token, e.g. `var(--accent)` or `var(--warn)`. */
  color: string
  ...
  return (
    <button
      ...
      style={{ background: `color-mix(in srgb, ${color} 10%, transparent)`, color, borderWidth: 1, borderColor: `color-mix(in srgb, ${color} 30%, transparent)` }}
      onClick={onClear}
      title={clearLabel}
      aria-label={clearLabel}
      data-testid={testId}
