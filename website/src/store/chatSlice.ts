...
interface ChatState {
  ...
  /** Per-slot count of turn STARTS this tab has seen (a user frame, the first
   *  busy frame after idle, a local send). Captured by `requestStop` before
   *  its request and checked by `settleStopNotRunning`, so a `not running`
   *  answer that was true when Stop was pressed cannot idle a NEWER turn — a
   *  cron or channel injection that started after the press — when it lands
   *  late (#9547, GPT round-2). */
...
type StopReply = { ok?: boolean; info?: string; already_stopping?: boolean } | null | undefined
