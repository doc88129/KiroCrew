  ...
  const handler = useCallback((e: KeyboardEvent) => {
    ...
    if (hit !== null) {
      ...
      const actions: Record<string, () => void> = {
        ...
        // session with a turn in flight always confirms. ⌘W/Ctrl+W is the most
        ...
        // keeps the user's confirm setting; a running one is where a stray
        // keystroke costs work, so it asks.
        'close-chat': () => {
          if (!activeSlot) return
          const running = !!slots.find(s => s.key === activeSlot)?.running
          const modChord = e.metaKey || e.ctrlKey
          const mustConfirm = loadChatConfig().confirmCloseSession || (modChord && running)
