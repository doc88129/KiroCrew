...
class VectorMemoryStore:
        ...
        *,
        ...
        with self._db_lock:
            ...
            try:
                ...
                if self.algorithm_version == "v2":
                    # A failed provenance stamp must not leave a pending INSERT
                    # for a later unrelated operation to commit without lineage.
                    self.db.rollback()
                raise
    ...
    def _ineligible_ids(self) -> set[str]:
        """Filter before rank/cap; validity is evaluated again on every recall."""
        rows = self._fetch_all_locked(
            "SELECT record_id, status, valid_from, valid_until FROM memory_record_meta "
            "WHERE status != 'active' OR valid_from != '' OR valid_until != ''"
        )
    ...
    def search_episodic(
        ...
        if (
            ...
            with self._db_lock:
                ...
                rows_by_id = self._get_episodic_batch(
                    [mem_id for mem_id, _ in hits], include_embedding=True
                )
                for mem_id, cosine_sim in hits:
                    ...
                    if blob is None or len(blob) != len(query_embedding) * 4:
                        continue
                    # The row body and vector must describe the same content,
                    # even if a caller forgot to invalidate an old index.
                    current = struct.unpack(f"{len(query_embedding)}f", blob)
                    cosine_sim = self._cosine_sim(list(current), query_embedding)
    ...
    def _sqlite_vector_search(
        ...
        if _HAS_NUMPY and not self._ineligible_ids():
    ...
    def _get_episodic_batch(
        self, mem_ids: list[str], *, include_embedding: bool = False
    ) -> dict[str, dict]:
        ...
        if not mem_ids:
            return {}
        columns = self._EPISODIC_SEARCH_COLUMNS + (", embedding" if include_embedding else "")
        out: dict[str, dict] = {}
    ...
    def recall(
        ...
        """
        from kiro_crew.memory_recall import bound_recall_payload, recall_evidence
        ...
        def fit(rows: list[dict], budget: int, *, episodic: bool) -> tuple[str, list[dict]]:
            ...
            for row in rows:
                ...
                line = f"[memory:{row['id']}] {body}\n"
                if len(line) > remaining:
                    continue
                chosen.append(recall_evidence(row, body, episodic=episodic))
                lines.append(line)
                remaining -= len(line)
        ...
        return bound_recall_payload(
            {
                ...
                "retrieval": {"facts": facts, "episodes": episodes},
                "semantic_chars": len(semantic),
                "episodic_chars": len(episodic),
                "lessons_chars": len(lessons),
                "total_chars": len(semantic) + len(episodic) + len(lessons),
