...
_EMAIL = re.compile(
    r"[\w.+-]+@[\w.-]+\.[\w-]+|\b(?:email|e-mail|gmail|outlook|inbox|smtp|imap)\b|邮箱|邮件", re.I
)
...
def _query(raw: Any) -> dict:
    if not isinstance(raw, dict) or set(raw) - {"q", "kind", "topic"}:
        raise MemoryEditError("Invalid memory filter.")
    query, kind, topic = raw.get("q", ""), raw.get("kind", "all"), raw.get("topic", "")
    if (
        not isinstance(query, str)
        or len(query) > 2000
        or not isinstance(kind, str)
        or kind not in _KINDS
        or topic not in ("", "email")
    ):
        raise MemoryEditError(
            "Use a valid kind, a query up to 2000 characters, and an optional email topic."
        )
    return {"q": query.strip(), "kind": kind, "topic": topic}
...
def _rows(store: Any, query: dict, identities: list[tuple[str, str]] | None = None):
    ...
    try:
        for row in cursor:
            ...
            if query["topic"] == "email" and not (
                meta.get("has_email") or _EMAIL.search(searchable)
            ):
                continue
...
def preview_edit(store: Any, store_name: str, secret: bytes, body: dict) -> dict:
    ...
    return {
        ...
        "entries": [
            ...
            for before, after in changes[:PREVIEW_ROWS]
        ],
        "preview_has_more": len(changes) > PREVIEW_ROWS,
        "warnings": [],
    }
