...
def _coerce_flag(raw: object, key: str) -> bool:
    ...
    if isinstance(raw, bool):
        return raw
    logger.warning(
        "admission policy %r is %s, not a boolean; treating it as True, the closed "
        "direction (write true or false, unquoted, or omit the key; releases before "
        "this rule read a null or empty value here as off)",
        key,
        type(raw).__name__,
    )
    return True
...
def _signature_valid(manifest: PluginManifest, policy: AdmissionPolicy) -> bool:
    ...
    # ``surrogatepass`` for the same reason: a JSON escape such as ``"\udc80"``
    # decodes to a lone surrogate, which a strict UTF-8 encode REFUSES with
    # ``UnicodeEncodeError`` -- a plain exception on the same escape path as the
    # ``TypeError`` above.  With it the bytes are simply not the digest's, and the
    # comparison answers ``False``.
    return hmac.compare_digest(
        expected.encode("utf-8"),
        str(manifest.signature).encode("utf-8", errors="surrogatepass"),
    )
