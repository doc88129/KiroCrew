...
#: The masked leaves that carry their own argument (see the sibling-gap note
#: above): DIRECTORIES the gateway creates on demand, whose EMPTY state is
#: absent-equivalent because nothing but the gateway reads them. Left to lazy
#: creation, a sandbox spawned on a fresh install finds the target absent, the
#: ``SENSITIVE_DIRS`` loop skips it, and the directory the gateway creates later
#: appears INSIDE that running sandbox's view -- where a same-UID agent can swap
#: it for a link. Materialised (empty, 0o700) before every namespace spawn
#: instead, so the mask always has a name to bind over.
#:
#: * ``aws-control-staging`` -- cut fresh per call for one CLI transfer and
#:   removed again; a writable name lets an agent swap the destination.
#: * ``crew-panels`` -- the crew webview records. The skip lands precisely on a
#:   fresh install, where the agent could create the directory itself and write
#:   what the gateway later reads back as authoritative. Same failure the
#:   ``panel-templates`` ceiling has, one list over.
_CREW_PRECREATE_HIDDEN_DIR_LEAVES: tuple[str, ...] = ("aws-control-staging", "crew-panels")
