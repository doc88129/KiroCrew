...
#:
#: Private memory has the same late-creation hazard: an agent spawned before the
#: first member must never gain access when that member's database appears.
...
def _prepare_private_log_dir() -> str:
...
def _private_memory_view_setup(log_directory: str = "") -> str:
    ...
    roots = repr(_private_memory_roots())
    return f"""        # Private member view: Global V1 remains exclusively gateway-owned.
        ...
        _private_views = []
        for _root in _private_roots:
            _private_views.extend([_root, os.path.join(_root, "workspace")])
        for _root in _private_views:
            ...
            for _name in os.listdir(_lower):
                ...
                if (_private_leaf(_name)
                        or (_root in _private_roots and _name in _private_broker_leaves)):
                    continue
                ...
                for _home in _private_roots:
                    if _resolved == _home:
                        _forbidden_alias = True
                        break
                    if _resolved.startswith(_home + os.sep):
                        _relative = _resolved[len(_home) + 1:]
                        _parts = _relative.split(os.sep)
                        _first = _parts[0]
                        if (_private_leaf(_first) or _first in _private_broker_leaves
                                or (_first == "agent-logs" and _name != "agent-logs")
                                or (_first == "workspace" and len(_parts) > 1 and _private_leaf(_parts[1]))
                                or (_relative == "workspace" and not (_name == "workspace" and _root == _home))):
                            _forbidden_alias = True
                            break
                if _forbidden_alias:
                    continue
                ...
                if os.path.isdir(_source):
                    ...
                    # A dangling nonmemory alias carries no global data; keep
                    # its spelling without opening a privileged source.
...
def _private_memory_seatbelt_rules(log_directory: str = "") -> list[str]:
    """Path predicates also cover files created after this profile is installed."""
    _validate_private_mcp_gateway_socket()
    rules = []
    for home in _private_memory_roots():
        for root in (home, home + "/workspace"):
            ...
            for operation in ("file-read*", "file-write*", "file-link"):
                rules.append(
                    f"(deny {operation} (regex {json.dumps(pattern, ensure_ascii=False)}))"
                )
...
def _build_launcher_script(
    ...
    *,
    ...
    private_setup = _private_memory_view_setup(private_log_dir) if private_memory else ""
...
def namespace_argv(
    ...
    *,
    ...
    private_log_dir = _prepare_private_log_dir() if private_memory else ""
    ...
    private_options: dict[str, Any] = (
        {"private_memory": True, "private_log_dir": private_log_dir} if private_memory else {}
    )
...
def _build_seatbelt_profile(
    ...
    *,
    ...
    if private_memory:
        ...
        rules.extend(_private_memory_seatbelt_rules(private_log_dir))
