...
def test_the_atomic_replace_still_refuses_a_planted_link() -> None:
    ...
    assert 0 <= shape_at < replace_at, (
        "the destination's shape is not judged before the report rename, so a planted link at "
        "the report path is overwritten instead of refused"
    )
...
def test_an_entry_that_cannot_be_inspected_refuses_instead_of_leaving_its_subtree_out(
    ...
    def _scandir(where):
        ...
        entries = list(real_scandir(where))
        if pathlib.Path(where) == root:
            return [_UninspectableEntry(e) for e in entries]
        return entries
...
def test_the_report_is_published_after_the_promotion_not_before() -> None:
    ...
    report_at = src.index("os.replace(report_tmp, report_path)")
    assert 0 <= promote_at < report_at, (
        "the report is written before the promotion completes, so a failed promotion can "
