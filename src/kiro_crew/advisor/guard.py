...
@dataclass
class UpdateBudget:
    """Non-blocker accounting for ONE reviewer update.

    Handed out by :meth:`EmissionGuard.begin_update` and held by the dispatch
    that owns the update. Dispatches for one parent overlap on the event loop
    (a blocker steer is awaited mid-update), so a counter kept on the shared
    guard would let one update reset or spend another's budget.
    """
...
class EmissionGuard:
    ...
    def admit(self, note: AdvisorNote, update: UpdateBudget) -> AdvisorNote | None:
        """Return the note (possibly escalated) if it may emit, else None.

        ``update`` is the budget of the reviewer update *note* belongs to.
        """
        if not _HAS_CONTENT.search(note.text):
            return None
        key = _normalize(note.text)
        previous = self._admitted.get(key)
        if previous is not None and _severity_rank(note.severity) <= _severity_rank(previous):
            return None
        # Either a new note or a higher-severity duplicate that escalates the
        # existing one. An escalation is a fresh card too, so it is subject to
        # the same non-blocker gates; only blockers are exempt.
        if note.severity != "blocker":
            if self._in_cooldown():
                return None
            if update.used >= self._non_blocker_budget:
                return None
            update.used += 1
        self._admitted[key] = note.severity
        return note

    def note_interruption(self, *, reserved: bool = False) -> None:
        """Record that an admitted blocker interrupted the primary.

        ``reserved=True`` confirms a slot :meth:`reserve_interruption` already
        counted, so the steer spends exactly one slot; the cooldown starts either
        way.
        """
    ...
    def reserve_interruption(self) -> bool:
        """Take one interruption slot now, or refuse when the cap is reached.

        Deliveries overlap on the event loop (each awaits its steer), so a check
        that counts only after the steer lands lets several pass a cap of one.
        The caller confirms with ``note_interruption(reserved=True)`` when the
        steer lands and :meth:`release_interruption` when it does not.
        """
        if not self.may_interrupt():
            return False
        self._interruptions_this_epoch += 1
        return True

    def release_interruption(self) -> None:
        """Return a reserved slot whose steer did not land."""
        if self._interruptions_this_epoch > 0:
            self._interruptions_this_epoch -= 1

    def begin_update(self) -> UpdateBudget:
        """Open a fresh non-blocker budget for one reviewer update."""
        return UpdateBudget()
