...
class AdvisorService:
    ...
    def __init__(self, enabled: bool = False, *, reviewer_available: bool = False) -> None:
        ...
        #: Liveness source for the orphan reclaim: the effective session keys
        #: of every slot the dashboard still holds (wired by the gateway at
        #: startup; None = no reclaim, the tests' default).
        self.live_session_keys: Callable[[], set[str]] | None = None
    ...
    def attach(
        ...
        if observer is None:
            self._reclaim_orphans(keep=parent_session_key)
    ...
    async def pump_async(self, state: object, slot: object) -> None:
        ...
        # update, so the observer keeps it for the next pump.
        try:
        ...
        payload: dict[str, object] = {
            ...
            # Named as text in the reviewer prompt so relative paths in the
            # supplied tool results retain their context.
    ...
    def notify_boundary(self, parent_session_key: str, reason: str) -> None:
        ...
        # A review already transmitted about the conversation this boundary
        # ends must stop now, not when the model decides to (close releases
        # the session below, which cancels too).
        cancel = getattr(self._pool, "cancel_active", None)
        if callable(cancel):
            cancel(parent_session_key)
        if reason == BOUNDARY_CLOSE:
    ...
    def _reclaim_orphans(self, *, keep: str) -> None:
        """Retire observers whose session no live slot fronts any more.

        Slots leave the registry through more than ``close_slot`` (ephemeral
        OpenAI-compatible completions, fork/transfer failure cleanups, history
        deletion pop them directly), and an observer left behind holds retained
        evidence and a reviewer pool session. Run when a NEW observer is about
        to be created -- the moment the registry grows -- through the same
        terminal boundary a close fires. ``keep`` is the session being
        attached: its slot may be published only after this turn starts.
        """
        source = self.live_session_keys
        if source is None:
            return
        try:
            live = set(source())
        except Exception:
            logger.debug("advisor: live session keys unavailable; no reclaim", exc_info=True)
            return
        for key in [k for k in self._observers if k != keep and k not in live]:
            self.notify_boundary(key, BOUNDARY_CLOSE)
...
def configure_from_config(cfg: object) -> None:
    ...
    service.reviewer_available = backend == ACP_BACKEND_KIRO
    if not service.reviewer_available:
        if service._enabled:
            logger.warning(
                "advisor: reviewer unavailable (agent.acp_backend=%r); the reviewer runs on "
                "the kiro-cli agent backend only -- advisor stays disabled",
                backend,
            )
    ...
    if not service._enabled:
        ...
        for key in [k for k, src in list(service._override_source.items()) if src != OVERRIDE_ON]:
            ...
            service._override_source.pop(key, None)
            service._boundary_gen[key] = service._boundary_gen.get(key, 0) + 1
            # The pool session goes with the observer: releasing it cancels a
            # review already transmitted and lets the runtime reap once the
            # explicit-on sessions are gone too.
            service._schedule_pool_release(key)
...
def set_advisor_override(slot: object, override: str) -> None:
    """Write a per-session override DECISION onto a slot: the value plus a bump
    of the slot's mutation token, so an older in-flight endpoint transaction
    (a save still awaiting) cannot roll this write back. Every writer
    that represents a user or peer decision goes through here; a restore from
    disk does not -- it is not a concurrent transaction."""
    slot.advisor_override = override  # type: ignore[attr-defined]
    slot._advisor_override_txn = int(getattr(slot, "_advisor_override_txn", 0) or 0) + 1  # type: ignore[attr-defined]
...
def notify_hard_kill(parent_session_key: str) -> None:
    ...
    if observer is None:
        ...
        return
    service._boundary_gen[parent_session_key] = service._boundary_gen.get(parent_session_key, 0) + 1
    # A review already transmitted keeps reading and spending until the
    # model stops on its own; the kill stops it now. The session stays.
    cancel = getattr(service._pool, "cancel_active", None)
    if callable(cancel):
        cancel(parent_session_key)
    ...
    # The user threw that work away -- seal observation (records + queued
    # final updates gone, late records from the runner being torn down
    # dropped, nothing completes) until the next turn attaches, and drop the
    # guard's window with it.
    observer.seal()
    service._guards.pop(parent_session_key, None)


def notify_history_rewrite(state: Any, slot: Any) -> list[tuple[Any, str, list[str]]]:
    """A history rewrite replaced the timeline the reviewer was looking at. Total.

    Regenerate, switch variant, edit-resend and rewind delete or replace turns
    on the live slot. A final review still running when the rewrite commits is
    about turns the rewrite removed, so it is discarded like a hard kill's,
    and advice already staged for those turns (every alias slot of the
    session) is cleared rather than injected into the replacement timeline.
    The rows a rewrite deletes take their Advisor cards with them.

    Returns a snapshot of what was cleared, for :func:`restore_staged_advice`:
    a rewrite whose durable save fails or is refused leaves the original
    timeline in place, and its staged advice must come back with it.
    """
    # circular import: dashboard.chat_runner imports this package's hooks.
    from kiro_crew.advisor.delivery import advisor_session_slots, clear_pending_advisor_context
    from kiro_crew.dashboard.chat_utils import effective_session_key

    snapshot: list[tuple[Any, str, list[str]]] = []
    try:
        notify_hard_kill(effective_session_key(slot))
        for member in advisor_session_slots(state, slot):
            staged = list(getattr(member, "_advisor_pending_context", None) or ())
            if staged:
                snapshot.append((member, effective_session_key(member), staged))
            clear_pending_advisor_context(member)
    except Exception:  # advisor must never break the primary rewrite
        logger.debug("advisor history-rewrite boundary failed", exc_info=True)
    return snapshot


def restore_staged_advice(snapshot: list[tuple[Any, str, list[str]]]) -> None:
    """Put back what :func:`notify_history_rewrite` cleared when the rewrite
    did not land: the original timeline stands, so its staged advice does too.
    Advice staged meanwhile (a preserve racing the failed save) stays, after
    the restored entries. A member rebound to another session while the save
    was out keeps nothing: the advice was about the session it showed then.
    Total."""
    from kiro_crew.dashboard.chat_utils import effective_session_key

    for member, session_key, staged in snapshot:
        try:
            if effective_session_key(member) != session_key:
                continue
            current = getattr(member, "_advisor_pending_context", None)
            if current is None:
                member._advisor_pending_context = list(staged)
            else:
                current[:0] = [entry for entry in staged if entry not in current]
            member._dirty = True
        except Exception:
            logger.debug("advisor staged-advice restore failed", exc_info=True)
