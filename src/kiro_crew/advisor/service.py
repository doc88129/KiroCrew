...
class AdvisorService:
    ...
    def __init__(self, enabled: bool = False, *, reviewer_available: bool = False) -> None:
        ...
        #: first review.
        # ``reviewer_available=True`` at construction stands for a host whose
        # startup probes answered yes (tests pin it that way); the gateway
        # constructs with the default and lets the probes fill these in.
        self.sandbox_available: bool | None = True if reviewer_available else None
        #: Whether the host's kiro-cli honours the preToolUse hook contract the
        #: read gate depends on (None = not probed yet).
        self.read_gate_available: bool | None = True if reviewer_available else None
...
def configure_from_config(cfg: object) -> None:
    ...
    # Exactly True: ``None`` is a probe that has not answered yet, and a live
    # PATCH landing in that window must not bind the reviewer on an unknown.
    service.reviewer_available = (
        backend == ACP_BACKEND_KIRO
        and service.sandbox_available is True
        and service.read_gate_available is True
    )
    if not service.reviewer_available:
        if service._enabled:
            logger.warning(
                "advisor: reviewer unavailable (agent.acp_backend=%r, sandbox_available=%r, "
                "read_gate_available=%r); the reviewer runs on the kiro-cli agent backend only, "
                "needs the strict sandbox's credential mask and a kiro-cli that honours the "
                "preToolUse hook contract -- advisor stays disabled",
                backend,
                service.sandbox_available,
                service.read_gate_available,
            )
