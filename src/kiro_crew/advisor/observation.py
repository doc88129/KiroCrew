...
class AdvisorObserver:
    ...
    def record_segment(self, text: str) -> None:
        ...
        if self._sealed:
            return  # a runner being torn down after a hard kill: dropped
    ...
    def seal(self) -> None:
        """Close observation until the next turn attaches (:meth:`begin_turn`).

        A hard kill discards the turn's work, and the runner it kills can still
        emit a queued tool result or its completion while the provider tears
        down. Sealed, those records are dropped, nothing completes, and no
        update is produced -- so no pump runs on evidence the user threw away.
        """
        self.begin_epoch()
        self._sealed = True
    ...
    def begin_turn(self) -> None:
        ...
        """
        if not self._completed and not self._sealed:
            return
