...
class AdvisorObserver:
    ...
    def __init__(
        ...
        self._dirty = False
        self._completed_update: ObservationUpdate | None = None
    ...
    def record_segment(self, text: str) -> None:
        ...
        self._segments.append(self._bound(text)[0])
        self._trim()
        self._dirty = True

    def record_reasoning(self, text: str) -> None:
        """Record already-redacted reasoning, bounded like every record.

        The caller owns the redaction boundary (the dashboard's own thinking
        redactor); the observer only bounds and stores.
        """
        self._require_active()
        self._reasoning.append(self._bound(text)[0])
        self._trim()
        self._dirty = True
    ...
    def begin_turn(self) -> None:
        ...
        if not self._completed:
            return
        pending = self._completed_update
        self.begin_epoch()
        self._completed_update = pending

    def take_completed(self) -> ObservationUpdate | None:
        """The held final update, exactly once (None when absent or taken)."""
    ...
    def _trim(self) -> None:
        ...
        the total -- three independently-full lists would triple it.
