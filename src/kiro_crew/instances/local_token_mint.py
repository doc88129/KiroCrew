...
secret travelled to whatever took loopback. ``validate_loopback_host`` narrows
the destination to ``127.0.0.1`` for the same reason, and this module re-checks
it at the send site: the two together mean the address the proof speaks for is
the address dialled.
    ...
    *embed_parent_port* becomes the minted token's signed CSP frame-ancestor
    ...
    if not is_self and not await asyncio.to_thread(port_is_gateway_owned_on_loopback, port):
        raise TokenMintError(
            ...
            f"{DEFAULT_LOOPBACK_HOST}:{port} (the pid sidecar in "
            ...
            f"instance at a gateway serving {DEFAULT_LOOPBACK_HOST} in this data "
