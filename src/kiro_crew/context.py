...
async def prepare_store_vectors(
    ...
    private = memory_store_version(name) == 2
    if private:
        ...
        if session_key:
            ...
            try:
                await asyncio.to_thread(bind_private_session_store, session_key, name)
...
def store_of_session(conversation_log: object, session_key: str) -> str:
    ...
    * The alternative a channel surface has in scope is its ``agent``, which carries
    ...
    try:
        ...
        return _resolved_store_name(meta.get("memory_store"))
    except Exception as exc:
