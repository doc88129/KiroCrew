...
def _validate_advisor_enabled(value: object, request: web.Request) -> str | None:
    ...
    if backend == ACP_BACKEND_KIRO:
        ...
        if get_advisor_service().read_gate_available is False:
            return (
                "The Advisor's reviewer gates its reads with a kiro-cli preToolUse hook and "
                "this kiro-cli is older than the release that contract was verified on "
                "(2.21.4); update kiro-cli"
            )
        if not get_advisor_service().reviewer_available:
            return "The Advisor's reviewer prerequisites are still being probed; retry shortly"
        return None
    return (
