...
def _peek_advisor_context(state: "DashboardState", slot: "_ChatSlot", full_message: str) -> str:
    ...
    if not staged:
        return full_message
    # The frame AND the outbound redaction are enforced HERE, where every
    # source of staged text (a live preserve, a sibling slot, a metadata
    # restore after restart) is rendered: an entry is redacted when produced,
    # but the persisted copy is disk state, so it leaves for the provider
    # through the same two redactors as every other advisory surface.
    notes = "\n".join(neutralize_reserved_delimiters(redact_outbound(entry)) for entry in staged)
    return (
        "[Advisor context]\n"
...
def _requeue_unconsumed_steers(state: "DashboardState", slot: "_ChatSlot") -> None:
    ...
    if _advisories:
        from kiro_crew.advisor.delivery import drop_unconsumed_advisory, preserve_advisory
        ...
        # An explicit opt-out landing after the steer is a disagreement with
        # the advice: it is not staged for the next (disabled) turn.
        _opted_out = getattr(slot, "advisor_override", "") == "off"
        ...
        for adv_msg in dict.fromkeys(_advisories):
            ...
            if isinstance(adv_env, _AdvEnv):
                if _opted_out:
                    drop_unconsumed_advisory(state, slot, adv_env)
                else:
...
async def _run_chat(
    ...
    *,
    ...
    try:
        ...
        # turn's checkpoints through the total observe_*/complete hooks. The
        # observer is keyed to the slot's LIVE conversation, and get_or_create
        # above is an await a rebind can land inside, so the binding is checked
        # again here: attaching after a rebind would feed this turn's events
        # into another conversation's reviewer context.
