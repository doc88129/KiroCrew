...
async def api_chat_slot_advisor_override(request: web.Request) -> web.Response:
    ...
    if override == "on":
        ...
        if not service.reviewer_available:
            ...
            if service.read_gate_available is False:
                return web.json_response(
                    {
                        "error": "The Advisor's reviewer gates its reads with a kiro-cli "
                        "preToolUse hook and this kiro-cli is older than 2.21.4",
                        "code": "advisor_read_gate_unavailable",
                    },
                    status=409,
                )
            if service.sandbox_available is None or service.read_gate_available is None:
                return web.json_response(
                    {
                        "error": "The Advisor's reviewer prerequisites are still being probed; "
                        "retry shortly",
                        "code": "advisor_probing",
                    },
                    status=409,
                )
            return web.json_response(
