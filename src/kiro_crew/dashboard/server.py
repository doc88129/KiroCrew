...
def _kick_advisor_configure(app: web.Application) -> None:
    ...
    async def _configure() -> None:
        ...
        def _live_session_keys() -> set[str]:
            # The orphan reclaim's liveness source: every session a live slot
            # fronts. Slots leave the registry through paths other than
            # close_slot, so observers are checked against this on attach.
            return {effective_session_key(s) for s in list(state._slots.values())}

        get_advisor_service().live_session_keys = _live_session_keys
        ...
        if get_advisor_service()._config_epoch != epoch_before:
            # A newer live configuration applied while the startup read was in flight;
            # re-apply that snapshot rather than the stale startup read.
            from kiro_crew.config import live
