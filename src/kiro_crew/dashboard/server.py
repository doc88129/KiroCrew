...
def _kick_advisor_configure(app: web.Application) -> None:
    ...
    async def _configure() -> None:
        ...
        # Whether this kiro-cli honours the preToolUse hook contract the
        # reviewer's read gate depends on (a version subprocess; off-loop).
        from kiro_crew.advisor.read_gate import probe_hook_contract

        get_advisor_service().read_gate_available = await asyncio.to_thread(probe_hook_contract)
        if get_advisor_service()._config_epoch != epoch_before:
            # A newer live configuration applied while the probes ran, on
            # probes that had not answered; re-apply THAT snapshot so the
            # probe verdicts reach it rather than the stale startup read.
            from kiro_crew.config import live
