    ...
    def _load_resolved(cls) -> tuple[KiroCrewConfig, int]:
        ...
        cfg = cls(
            ...
            memory=MemoryConfig(
                ...
                embedding_threads=_safe_int(memory_data.get("embedding_threads", 1), 1, 1, 2),
                ...
                embedding_bulk_threads=_safe_int(
                    memory_data.get("embedding_bulk_threads", 1), 1, 0, 2
                ),
                ...
                backup_enabled=memory_data.get("backup_enabled", True),
    ...
    def create_provider_factory(self) -> Callable:
        ...
        def _acp(
            ...
            **_kwargs: object,
            ...
            _private_store = private_memory_store_for_session(session_key)
            from typing import Any

            _private_kwargs: dict[str, Any] = {"private_memory": True} if _private_store else {}
            return AcpProvider(
                work_dir=wdir,
                model=m,
                agent=agent,
...
def resolve_agent_bindings(
    ...
    *project_dir* is the session's active project directory, which widens step 2 to
    ...
    # Only the built-in default assistant owns V1. Selecting a named member as
    # the preferred agent never turns its private memory into global memory.
    store_name = (
        DEFAULT_MEMORY_STORE
        if passthrough
